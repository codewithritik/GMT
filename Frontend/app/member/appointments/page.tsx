...
function slotOverlapsBusy(
  ...
  return (
    timeToMinutes(slotStart) < timeToMinutes(busyEnd) &&
    timeToMinutes(busyStart) < timeToMinutes(slotEnd)
  );
}

function slotInsideWorkingHours(
  slotStart: string,
  slotEnd: string,
  windows: TrainerPtAvailability["workingWindows"],
): boolean {
  if (!windows.length) return true;
  const a = timeToMinutes(slotStart);
  const b = timeToMinutes(slotEnd);
  return windows.some(
    (w) => a >= timeToMinutes(w.start) && b <= timeToMinutes(w.end),
  );
}

function slotInsideGymHours(
  slotStart: string,
  slotEnd: string,
  gymOpen: string,
  gymClose: string,
...
export default function AppointmentsPage() {
  ...
  const [availability, setAvailability] =
    useState<TrainerPtAvailability | null>(null);
  ...
  const loadSessions = async () => {
    try {
      ...
      const trainerMap = new Map(
        (trainers ?? []).map((t: any) => [t.id, t.fullName]),
      );
      setSessions(
        (rawSessions ?? []).map((s: any) => ({
          ...
          trainerName:
            s.trainerName ?? trainerMap.get(s.trainerId) ?? s.trainerId,
          ...
          durationMinutes:
            ...
            Number.isFinite(Number(s.durationMinutes))
              ...
              : Math.max(
                  0,
                  timeToMinutes(String(s.endTime).slice(0, 5)) -
                    timeToMinutes(String(s.startTime).slice(0, 5)),
                ),
          status: s.status,
      ...
      setTrainerOptions(
        (trainers ?? []).map((t: any) => ({ value: t.id, label: t.fullName })),
      );
    } catch {
  ...
  const slotCheck = (() => {
    ...
    if (availability?.bookingRules?.isClosedDay) {
      return {
        tone: "bad" as const,
        message: "The gym is closed on this date. Choose another day.",
      };
    }
    if (form.time) {
      ...
      if (Number.isFinite(sm) && sm + form.durationMinutes > 24 * 60) {
        return {
          tone: "bad" as const,
          message:
            "Session extends past midnight; choose an earlier start or shorter duration.",
        };
      }
    }
    ...
    const clashes = availability.busySlots.some((b) =>
      slotOverlapsBusy(startShort, endShort, b.startTime, b.endTime),
    );
    const inHours = slotInsideWorkingHours(
      startShort,
      endShort,
      availability.workingWindows,
    );
    if (clashes)
      return {
        tone: "bad" as const,
        message:
          "This time overlaps another booking for this trainer. Pick a different slot.",
      };
    if (availability.hasShift && !inHours)
      return {
        tone: "warn" as const,
        message:
          "Outside this trainer’s listed shift for that day—they may still confirm manually.",
      };
    if (!availability.hasShift)
      return {
        tone: "warn" as const,
        message:
          "No shift on file for this day; you can still request a session and the trainer will confirm.",
      };
    return {
      tone: "ok" as const,
      message:
        "This time is inside their shift, within gym hours, and does not overlap existing bookings.",
    };
  })();
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      toast.success(
        "Booking Confirmed",
        `Session booked on ${form.date} at ${form.time} (${form.durationMinutes} min)`,
      );
      setShowModal(false);
      setForm({
        trainer: "",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-2 flex-wrap">
        {(
          ["all", "booked", "confirmed", "completed", "cancelled"] as const
        ).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <div className="text-center py-12 text-zinc-600">
            No {displayFilter(filter)} appointments.
          </div>
        )}
        {filtered.map((s) => (
          <Card
            key={s.id}
            padding="md"
            className="flex flex-col gap-3 min-w-0 overflow-hidden hover:border-zinc-700/50 transition-colors">
              ...
              <div className="flex min-w-0 w-full flex-col gap-2 sm:w-auto sm:max-w-[min(100%,20rem)] sm:items-end sm:shrink-0">
                <div className="text-right sm:min-w-[9rem]">
                  ...
                  <p className="text-zinc-500 text-xs flex items-center gap-1 justify-end flex-wrap">
                    <Clock size={11} className="shrink-0" /> {s.startTime} –{" "}
                    {s.endTime}
                    {s.durationMinutes != null ? (
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-semibold capitalize ${statusStyles[s.status]}`}>
                    {displayFilter(s.status)}
                  </span>
                  {isUpcoming(s) && (
                    <button
                      onClick={() =>
                        setCancelModal({ id: s.id, trainerName: s.trainerName })
                      }
            ...
            {s.status === "completed" && s.reviewRating != null && (
              <div className="mt-3 pt-3 border-t border-zinc-800/80 text-sm text-zinc-400">
                ...
                {s.reviewComment ? (
                  <p className="mt-1 text-zinc-500">{s.reviewComment}</p>
                ) : null}
              </div>
            )}
          </Card>
      ...
      <Modal
        ...
        description="See the trainer’s shift and existing bookings, then pick a time."
        size="lg">
        <div className="space-y-4">
          <Select
            id="appointments-trainer"
            label="Trainer"
          ...
          {bookingRules && (
            <p className="text-zinc-500 text-xs -mt-2">
              Branch hours {bookingRules.gymOpen}–{bookingRules.gymClose} (
              {bookingRules.timezone}). Book up to {bookingRules.advanceDaysMax}{" "}
              days ahead.
            </p>
          )}
          {form.trainer && form.date && (
            <div className="rounded-xl border border-zinc-700/80 bg-zinc-900/40 p-4 text-sm">
              <p className="text-zinc-400 text-xs font-semibold uppercase tracking-wide mb-2">
                Trainer availability — {form.date}
              </p>
              {availabilityLoading && (
                  ...
                  <div>
                    ...
                    {availability.workingWindows.length === 0 ? (
                      <p className="text-amber-400/90 text-sm">
                        No published shift. You can still book; the trainer
                        confirms when they see it.
                      </p>
                    ) : (
                  ...
                  <div>
                    ...
                    {availability.busySlots.length === 0 ? (
                      <p className="text-zinc-400 text-sm">
                        No overlapping sessions — full day open aside from shift
                        boundaries.
                      </p>
                    ) : (
                      <ul className="text-zinc-300 text-sm space-y-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Select
              ...
              label="Duration"
              options={SESSION_DURATION_CHOICES.map((n) => ({
                value: String(n),
                label: `${n} minutes`,
              }))}
          ...
          {slotCheck && (
            <p
              className={`text-sm ${slotCheck.tone === "ok" ? "text-emerald-400" : slotCheck.tone === "warn" ? "text-amber-400" : "text-rose-400"}`}>
              {slotCheck.message}
            </p>
          )}
      ...
      <Modal
        ...
        description={
          rateModal
            ? `How was your session with ${rateModal.trainerName}?`
            : undefined
        }
        size="sm">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <LoadingButton loading={rateLoading} onClick={handleSubmitReview}>
              Submit review
            </LoadingButton>
          </div>
        </div>
