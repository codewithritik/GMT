  ...
  const handleSubmit = async () => {
    ...
    try {
      await authAPI.completeOnboarding({
        experienceLevel: data.experienceLevel as
          | "beginner"
          | "intermediate"
          | "advanced",
  ...
  return (
    <div className="min-h-screen bg-app text-white flex flex-col items-center justify-center p-6 relative overflow-hidden selection:bg-red-600/30">
      ...
      <div className="w-full max-w-xl relative z-10">
        ...
        <div
          className="bg-zinc-800/80 backdrop-blur-xl border border-zinc-700 rounded-3xl p-8 shadow-2xl"
          id="onboard-card">
          {error && (
            <div
          ...
          {step === 1 && (
            <div className="space-y-5" id="onboard-step-1">
              ...
              <div
                className={cn(
                  "grid gap-4",
                  needsTwoFiles ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1",
                )}>
            ...
            <div className="space-y-5" id="onboard-step-2">
              <div>
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Dumbbell size={22} className="text-red-500 shrink-0" />{" "}
                  Experience
                </h2>
              ...
              <div
                ...
                role="group"
                aria-labelledby="onboard-experience-label">
                <span id="onboard-experience-label" className="sr-only">
                  Experience level
                </span>
                {[
          ...
          {step === 3 && (
            <div className="space-y-5" id="onboard-step-3">
              <div>
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Target size={22} className="text-red-500 shrink-0" /> Fitness
                  Goals
                </h2>
                <p className="text-zinc-400 text-sm mt-1">
                  Pick one or more goals (tap to select/deselect).
                </p>
              </div>
              <div
                className="grid grid-cols-1 sm:grid-cols-2 gap-2"
                role="group"
                aria-labelledby="onboard-goals-label">
                <span id="onboard-goals-label" className="sr-only">
                  Fitness goals
                </span>
                {GOALS.map((goal) => {
                  const selected = data.fitnessGoals
                    .split(",")
                    .includes(goal.id);
                  return (
                    <button
                      ...
                      <goal.icon
                        size={18}
                        className={selected ? "text-red-400" : "text-zinc-400"}
                      />
