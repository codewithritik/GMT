...
export default function MemberSubscriptionPage() {
  ...
  const [validTrainerRefs, setValidTrainerRefs] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      opsAPI
        .mySubscriptions()
        .then((d) => setSubscriptions((d ?? []) as MySubscription[])),
        ...
        if (usersRes.status === "fulfilled") {
          for (const raw of usersRes.value ?? []) {
            ...
            const id = String(t.id ?? "")
              .trim()
              .toLowerCase();
            const code = String(t.employeeCode ?? "")
              .trim()
              .toLowerCase();
            if (id) refs.add(id);
  ...
  const activeSubscription =
    subscriptions.find((s) => ["active", "grace_period"].includes(s.status)) ??
    subscriptions[0];
  const planOptions = plans.map((p) => ({
    value: p.id,
    label: `${p.name} — ${p.name} — ${Number(p.price ?? 0).toLocaleString("en-US")}`,
  }));
  ...
  const submitCheckout = async () => {
    ...
    if (
      ...
      checkout.cardPan.replace(/\D/g, "").length < 13
    ) {
    ...
    if (
      checkout.paymentMethod === "card" &&
      !/^\d{2}\/\d{2}$/.test(checkout.cardExpiry)
    ) {
    ...
    if (
      checkout.paymentMethod === "card" &&
      checkout.cardCvv.replace(/\D/g, "").length < 3
    ) {
    ...
    try {
      ...
      toast.success(
        "Payment Successful",
        "Subscription activated and invoice issued.",
      );
  ...
  const validateReferralCode = (referralRaw: string): boolean => {
    ...
    if (isUuid && trainerIdsLoaded) {
      toast.error(
        "Validation Error",
        "Referral Code must be a valid trainer id/code",
      );
      return false;
    }
    if (!isUuid && !isTrainerCodeFormat) {
      toast.error(
        "Validation Error",
        "Referral must be a trainer UUID or code like GMS-TRN-001",
      );
      return false;
    }
    return true;
        ...
        <div
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">
              You cannot purchase, renew, or upgrade a subscription until your
              identity is verified. Please resubmit your ID documents from your{" "}
              <Link
                href="/member/profile"
      ...
      {!profileLoading && idPending && (
        <div
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">
              Your identity documents are being reviewed. Subscription purchases
              are blocked until your ID is approved.
            </p>
          </div>
        </div>
      )}
            ...
            <Card padding="lg" className="text-center space-y-5">
              ...
              <div>
                ...
                <p className="text-zinc-400 text-sm mt-1">
                  Purchase your first subscription to access GymSphere services.
                </p>
              </div>
              <LoadingButton
                icon={CreditCard}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-400 text-sm mt-1">
                      $
                      {Number(activeSubscription.pricePaid ?? 0).toLocaleString(
                        "en-US",
                      )}
                    </p>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            onChange={(e) =>
              setRenewForm((f) => ({ ...f, promo: e.target.value }))
            }
          />
          <Input
            label="Referral Code"
            ...
            onChange={(e) =>
              setRenewForm((f) => ({ ...f, referredBy: e.target.value }))
            }
          />
          <Select
            ...
            onChange={(e) =>
              setRenewForm((f) => ({ ...f, payment: e.target.value }))
            }
          />
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleRenew}>
              Confirm Renewal
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          <Select
            ...
            onChange={(e) =>
              setCheckout((f) => ({ ...f, paymentMethod: e.target.value }))
            }
          />
          <Input
            label="Card Number"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={submitCheckout}>
              Pay
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          <Select
            ...
            onChange={(e) =>
              setUpgradeForm((f) => ({ ...f, plan: e.target.value }))
            }
            placeholder="Choose a plan"
          />
          <Input
            ...
            onChange={(e) =>
              setUpgradeForm((f) => ({ ...f, referredBy: e.target.value }))
            }
          />
          <p className="text-xs text-zinc-500">
            Prorated amount will be calculated at checkout.
          </p>
          <div className="flex justify-end gap-3 pt-2">
            <LoadingButton
              variant="secondary"
            ...
            <LoadingButton loading={loading} onClick={handleUpgrade}>
              Upgrade
            </LoadingButton>
          </div>
        </div>
