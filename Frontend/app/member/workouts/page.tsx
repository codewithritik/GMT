...
function PlanSourceTag({
  ...
  return (
    <span
      className={`inline-block text-[10px] px-1.5 py-0.5 rounded-full font-semibold mt-0.5 ${cls} ${dark}`}>
      {label}
    </span>
  );
}
      ...
      <div onClick={onSelect} className="cursor-pointer">
        <Card
          ...
          className={`overflow-hidden transition-all hover:scale-[1.01] ${
            selected
              ? "border-red-600/60 bg-red-600/5 ring-1 ring-red-500/35"
          ...
          <div className="relative h-44 sm:h-48">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 pt-14">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-white/90 tabular-nums">
                <span
                  className={`rounded-full px-2 py-0.5 font-semibold capitalize ${difficultyColor[plan.difficulty ?? "beginner"]}`}>
                  {plan.difficulty ?? "beginner"}
                </span>
                <span className="flex items-center gap-1">
                  <Clock size={12} className="opacity-90" />{" "}
                  {plan.durationWeeks} wk
                </span>
                <span className="flex items-center gap-1">
                  <Flame size={12} className="opacity-90" /> {plan.daysPerWeek}
                  ×/wk
                </span>
    ...
    <div onClick={onSelect}>
      <Card
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <span
            className={`shrink-0 text-[10px] px-2 py-0.5 rounded-full font-semibold capitalize ${difficultyColor[plan.difficulty ?? "beginner"]}`}>
            {plan.difficulty ?? "beginner"}
          </span>
        </div>
        <div className="flex gap-4 text-xs text-zinc-500">
          <span className="flex items-center gap-1">
            <Clock size={11} /> {plan.durationWeeks} weeks
          </span>
          <span className="flex items-center gap-1">
            <Flame size={11} /> {plan.daysPerWeek} days/week
          </span>
        </div>
      </Card>
    </div>
...
export default function WorkoutsPage() {
  ...
  const [planToRemove, setPlanToRemove] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const loadPlans = useCallback(() => {
    opsAPI
      ...
      .catch(() =>
        toastRef.current.error("Error", "Failed to load workout plans"),
      );
  }, []);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("pw:workout-plans-refresh", onRefresh);
  }, [loadPlans]);
    ...
    window.dispatchEvent(
      new CustomEvent("pw:ai-chat-prefill", {
        detail: {
          ...
          message: AI_WORKOUT_PLAN_CHAT_INTRO,
          resetSession: true,
        },
      }),
    );
  ...
  const confirmRemovePlan = async () => {
    ...
    try {
      ...
      toast.success(
        "Plan removed",
        "This programme is no longer in My programmes.",
      );
  ...
  const handleLogCompleted = async () => {
    ...
    const durationMin = logDuration.trim()
      ? Math.max(1, Number(logDuration))
      : undefined;
    ...
    try {
      ...
      toast.success(
        "Workout logged",
        "Your session was saved to your history.",
      );
      setLogOpen(false);
      setLogDuration("");
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-8">
              ...
              {loadingDetail && selected ? (
                    ...
                    <div className="relative min-h-[220px] sm:min-h-[280px] md:min-h-[300px]">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div className="min-w-0 pr-2">
                          <h2 className="text-2xl md:text-4xl font-bold text-white tracking-tight leading-tight drop-shadow-lg">
                            {active.name}
                          </h2>
                          <p className="mt-1.5 text-sm text-white/85">
                            {active.durationWeeks} weeks · {active.daysPerWeek}{" "}
                            days/week ·{" "}
                          ...
                          <div className="flex flex-wrap gap-2 shrink-0">
                            <LoadingButton
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  if (activeSession?.id) {
                                    await opsAPI.stopWorkoutSession(
                                      activeSession.id,
                                      { complete: true, mood: "good" },
                                    );
                                    toast.success(
                                      "Workout completed",
                                    ...
                                    const s = await opsAPI.startWorkoutSession({
                                      planId: active.id,
                                      notes: `Started from plan: ${active.name}`,
                                    });
                                    toast.success(
                                      "Workout started",
                                      "Session is live — tap Complete when you are done.",
                                    );
                                    setActiveSession(s);
                                  }
                                } catch (err) {
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    if (activeSession?.id) {
                                      ...
                                      const s =
                                        await opsAPI.startWorkoutSession({
                                          planId: active.id,
                                          notes: `Started from plan: ${active.name}`,
                                        });
                                      toast.success(
                  ...
                  <div className="p-6">
                    {isAssignedPlan && logOpen && (
                      <div className="mb-4 rounded-xl border border-zinc-700/80 bg-zinc-900/50 p-4 space-y-3">
                        ...
                        <p className="text-zinc-500 text-xs">
                          Saves to your history without using a live timer.
                          Optional duration refines your log.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <select
                              value={logMood}
                              onChange={(e) =>
                                setLogMood(e.target.value as typeof logMood)
                              }
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <button
                            ...
                            className="text-xs px-3 py-2 text-zinc-400 hover:text-white">
                            Cancel
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="mb-6 flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() => {
                          const detail = {
                            role: "member" as const,
                            message: `Explain my plan "${active.name}" and what I should focus on this week.`,
                          };
                          window.dispatchEvent(
                            new CustomEvent("pw:ai-chat-prefill", { detail }),
                          );
                        }}
                    ...
                    <div className="space-y-3">
                      <div className="bg-zinc-800/30 rounded-xl p-4">
                        ...
                        {programIntro ? (
                          <p className="text-zinc-300 text-sm mb-3 leading-relaxed whitespace-pre-wrap">
                            {programIntro}
                          </p>
                        ) : null}
                        {active.description ? (
                      ...
                      <div className="bg-zinc-800/30 rounded-xl p-4">
                        ...
                        {days.length === 0 ? (
                          <p className="text-zinc-500 text-xs">
                            No structured days in this programme yet.
                          </p>
                        ) : (
                                ...
                                <div
                                  ...
                                  {isOpen && (
                                    <div className="divide-y divide-zinc-800/50">
                                      {day.notes ? (
                                        <p className="px-4 py-3 text-xs text-zinc-400 bg-zinc-900/35 border-b border-zinc-800/60 leading-relaxed">
                                          {day.notes}
                                        </p>
                                      ) : null}
                                      {exs.map((ex, i) => (
                                            ...
                                            {!hideExerciseThumbs &&
                                              ex.imageUrl && (
                                                <img
                                                  src={ex.imageUrl}
                                                  alt=""
                                                  className="w-full sm:w-28 h-20 object-cover rounded-lg border border-zinc-700/50 shrink-0"
                                                />
                                              )}
                                            <div className="min-w-0 flex-1">
                                              <div className="flex items-start justify-between gap-3">
                                                ...
                                                <div className="flex flex-wrap gap-2 shrink-0 text-xs justify-end">
                                                  {ex.sets != null && (
                                                    <span className="bg-zinc-700 text-zinc-300 px-2 py-0.5 rounded">
                                                      {ex.sets} sets
                                                    </span>
                                                  )}
                                                  {ex.reps != null && (
                ...
                <Card className="flex flex-col items-center justify-center min-h-[280px] text-zinc-500 text-sm text-center px-6">
                  <p className="text-zinc-400">
                    Choose a programme from the list to see exercises and
                    actions.
                  </p>
                </Card>
              )}
      ...
      <Modal
        isOpen={!!planToRemove}
        onClose={() => {
          if (!removing) setPlanToRemove(null);
        }}
