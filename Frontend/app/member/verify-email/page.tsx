...
function VerifyEmailContent() {
  ...
  const [status, setStatus] = useState<
    "loading" | "success" | "error" | "pending" | "email_failed"
  >("loading");
  ...
  useEffect(() => {
    ...
    if (
      sentParam === "1" ||
      (isAuthenticated && user?.role === "member" && !user?.emailVerified)
    ) {
      setStatus("pending");
      setMessage(
        sentParam === "1"
      ...
      {status === "success" && (
        <>
          ...
          <p className="text-zinc-400 mb-8 leading-relaxed">
            Your account has been successfully verified. Continue to complete
            your profile.
          </p>
          <button
