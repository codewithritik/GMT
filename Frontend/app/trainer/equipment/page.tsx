...
export default function TrainerEquipmentPage() {
  ...
  const openCountByEq = useMemo(() => {
    ...
    reports
      ...
      .forEach((r) => m.set(r.eqId, (m.get(r.eqId) ?? 0) + 1));
    return m;
  }, [reports]);
  ...
  const handleSubmit = async () => {
    if (!form.equipment || !form.issue.trim()) {
      toast.error(
        "Validation Error",
        "Please select equipment and describe the issue",
      );
      return;
    }
  ...
  const eqNameById = useMemo(
    () => new Map(equipment.map((e) => [e.id, e.name])),
    [equipment],
  );
  const EQUIPMENT_OPTIONS = equipment.map((e) => ({
    value: e.id,
    label: e.name,
  }));
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {equipment.map((eq) => (
          <div
            key={eq.id}
            className={`bg-zinc-900/50 border rounded-2xl p-5 ${statusColor[eq.status].split(" ")[2] || "border-zinc-800"}`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full font-semibold border ${statusColor[eq.status]}`}>
                {eq.status.replace("_", " ")}
              </span>
            </div>
            <p className="text-zinc-600 text-xs">Last maintenance: —</p>
            {(openCountByEq.get(eq.id) ?? 0) > 0 && (
              <p className="text-red-400 text-xs mt-1">
                {openCountByEq.get(eq.id)} open{" "}
                {(openCountByEq.get(eq.id) ?? 0) === 1 ? "issue" : "issues"}
              </p>
            )}
          </div>
        ))}
        ...
        <div className="space-y-3">
          {reports
            ...
            .map((r, i) => (
              <div
                key={i}
                className="flex items-start justify-between bg-zinc-800/30 rounded-xl p-4">
                <div>
                  <p className="text-white text-sm font-semibold">
                    {eqNameById.get(r.eqId) ?? "Equipment"} — {r.issue}
                  </p>
                  <p className="text-zinc-500 text-xs">Reported on {r.date}</p>
                </div>
                <span
                  className={`text-xs px-2 py-1 rounded-full font-semibold ${reportStatusColor[r.status]}`}>
                  {r.status.replace("_", " ")}
                </span>
              </div>
            ))}
