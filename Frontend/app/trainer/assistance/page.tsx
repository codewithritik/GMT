...
export default function TrainerAssistancePage() {
  ...
  useEffect(() => {
    load().catch((err) =>
      toast.error("Failed to load assistance data", getErrorMessage(err)),
    );
  }, []);
  ...
  return (
    <div className="space-y-8">
      ...
      <Card padding="lg">
        ...
        {filteredEq.length === 0 && (
          <p className="text-zinc-500 text-sm">
            No equipment items in this filter.
          </p>
        )}
            ...
            <Card
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-3 gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-semibold border ${priorityColor[r.priority]}`}>
                    {r.priority}
                  </span>
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${statusColor[r.status]}`}>
                    {r.status.replace("_", " ")}
                  </span>
                </div>
              </div>
      ...
      <Card padding="lg">
        ...
        {notices.length === 0 && (
          <p className="text-zinc-500 text-sm">
            No recent broadcast notices. Override requests from your schedule
            are logged here for managers.
          </p>
        )}
