  ...
  const loadTasks = async () => {
    ...
    const fromEvents: Task[] = (events ?? [])
      ...
      .map((e: any, i: number) => ({
        ...
        priority: (e.severity === "critical"
          ? "high"
          : (e.severity ?? "medium")) as Priority,
        eta: "30 min",
    ...
    <div className="space-y-8">
      ...
      <Card padding="md">
        ...
        <div className="w-full bg-zinc-800 rounded-full h-3">
          <div
            className="bg-gradient-to-r from-orange-600 to-orange-400 h-3 rounded-full transition-all"
            style={{ width: `${pct}%` }}
          />
        </div>
          ...
          <div
            ...
            <Card
              ...
              <div className="flex items-start gap-4">
                <div
                  className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${t.status === "completed" ? "border-green-500 bg-green-500/20" : "border-zinc-600"}`}>
                  {t.status === "completed" && (
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <p
                      className={`font-semibold text-sm ${t.status === "completed" ? "line-through text-zinc-500" : "text-white"}`}>
                      {t.task}
                    </p>
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ml-2 flex-shrink-0 ${priorityColor[t.priority]}`}>
                      {t.priority}
                    </span>
                  </div>
