...
export default function TrainerDashboard() {
  ...
  const [recentCheckIns, setRecentCheckIns] = useState<
    Array<{ member: string; time: string; type: CheckInType }>
  >([]);
  const [pendingTasks, setPendingTasks] = useState<
    Array<{ task: string; priority: Priority; eta: string }>
  >([]);
  const [equipment, setEquipment] = useState<
    Array<{ name: string; status: EquipmentStatus; last: string }>
  >([]);
  ...
  const refresh = async () => {
    const [dash, visits, equipmentRows, eventRows, myVisits] =
      await Promise.all([
    ...
    setEquipment(
      (equipmentRows ?? []).slice(0, 6).map((e: any) => ({
        name: e.name,
        status:
          e.status === "needs_maintenance"
            ? "maintenance"
            : e.status === "retired"
              ? "out_of_order"
            ...
            <div
              ...
              <div>
                ...
                <p className="text-zinc-500 text-xs">
                  Last maintenance: {e.last}
                </p>
              </div>
              <span
                className={`text-xs px-2 py-1 rounded-full font-semibold ${statusColor[e.status]}`}>
                {e.status.replace("_", " ")}
              </span>
            </div>
          ))}
