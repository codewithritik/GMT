...
export default function TrainerMembersPage() {
  ...
  const loadMembers = async () => {
    ...
    (visits ?? []).forEach((v: any) => {
      if (!latestVisit.has(v.personId)) {
        latestVisit.set(
          v.personId,
          new Date(v.checkInAt ?? v.createdAt).toLocaleDateString(),
        );
      }
    });
    setMembers(
  ...
  const openDetail = async (m: MemberRow) => {
    ...
    try {
      const [metrics, plans, sessions] = await Promise.all([
        ...
        opsAPI
          .allPtSessions()
          .then((all: any[]) => all.filter((s: any) => s.memberId === m.id)),
      ]);
      setDetail({
        member: m,
        metrics: metrics ?? [],
  ...
  const handleVitals = async () => {
    ...
    try {
      ...
      await opsAPI.addMemberMetric(selectedMember.id, {
        ...
        bmi,
        notes:
          `BF%=${vitalsForm.bodyFat || "-"} ${vitalsForm.notes || ""}`.trim(),
      });
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      await opsAPI.assignWorkoutPlan({
        ...
        ...(assignForm.libraryPlanId
          ? { libraryPlanId: assignForm.libraryPlanId }
          : {}),
      });
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="My Members"
        subtitle="Assigned members — log vitals, assign workouts, view history"
      />

      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <SearchInput
          id="trainer-members-search"
          value={search}
          onChange={setSearch}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((m) => (
          <Card
            key={m.id}
            padding="md"
            className="hover:border-zinc-700/50 transition-colors">
            <div className="flex items-start justify-between mb-4">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-semibold ${m.status === "active" ? "bg-emerald-500/20 text-emerald-400" : "bg-zinc-500/20 text-zinc-400"}`}>
                {m.status}
              </span>
            </div>
          ...
          detail && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 border-b border-zinc-800 pb-3">
                {(["metrics", "plans", "sessions"] as DetailTab[]).map(
                  (tab) => (
                    <button
                      key={tab}
              ...
              {detailTab === "metrics" && (
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  ...
                  {detail.metrics.map((m: any, i: number) => (
                    <div
                      key={i}
                      className="bg-zinc-800/30 rounded-xl p-3 grid grid-cols-4 gap-3 text-sm">
                      <div>
                        ...
                        <div className="p-3 flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-zinc-500 text-xs">
                              {p.daysPerWeek} days/week · {p.durationWeeks}{" "}
                              weeks ·{" "}
                        ...
                        {isExpanded && (
                          <div className="border-t border-zinc-700/50 p-3 space-y-2">
                            {!detailRow ? (
                              ...
                              <p className="text-zinc-500 text-xs">
                                No programme days in this plan yet.
                              </p>
                            ) : (
                              days.map((day: any) => {
              ...
              {detailTab === "sessions" && (
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  ...
                  {detail.sessions.map((s: any, i: number) => (
                    <div
                      key={i}
                      className="bg-zinc-800/30 rounded-xl p-3 flex justify-between items-center">
                      <div>
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${s.status === "completed" ? "bg-zinc-500/20 text-zinc-400" : s.status === "cancelled" ? "bg-red-500/20 text-red-400" : "bg-blue-500/20 text-blue-400"}`}>
                        {s.status}
                      </span>
                    </div>
                  ))}
      ...
      <Modal
        ...
        title="Log Member Vitals"
        description={
          selectedMember ? `Record vitals for ${selectedMember.name}` : ""
        }
        size="md">
        <div className="space-y-4">
          <Input
            id="trainer-vitals-weight"
            label="Weight (kg)"
            type="number"
            value={vitalsForm.weight}
            onChange={(e) =>
              setVitalsForm((f) => ({ ...f, weight: e.target.value }))
            }
            placeholder="e.g. 75"
          />
          <Input
            id="trainer-vitals-height"
            label="Height (cm)"
            type="number"
            value={vitalsForm.height}
            onChange={(e) =>
              setVitalsForm((f) => ({ ...f, height: e.target.value }))
            }
            placeholder="e.g. 175"
          />
          <Input
            id="trainer-vitals-body-fat"
            label="Body Fat % (optional)"
            type="number"
            value={vitalsForm.bodyFat}
            onChange={(e) =>
              setVitalsForm((f) => ({ ...f, bodyFat: e.target.value }))
            }
            placeholder="e.g. 18"
          />
          <Input
            id="trainer-vitals-notes"
            label="Notes (optional)"
            value={vitalsForm.notes}
            onChange={(e) =>
              setVitalsForm((f) => ({ ...f, notes: e.target.value }))
            }
            placeholder="Notes"
          />
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleVitals}>
              Save
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        description={
          selectedMember ? `Assign plan to ${selectedMember.name}` : ""
        }
        size="md">
        <div className="space-y-4">
          <Select
              ...
              setAssignForm((f) => ({
                ...
                difficulty:
                  (lib?.difficulty as
                    | "beginner"
                    | "intermediate"
                    | "advanced") ?? f.difficulty,
              }));
            }}
          />
          <Input
            id="trainer-assign-plan"
            label="Workout Plan Name"
            value={assignForm.plan}
            onChange={(e) =>
              setAssignForm((f) => ({ ...f, plan: e.target.value }))
            }
            placeholder="e.g. Fat Loss Starter - Week 1"
          />
          <Input
            id="trainer-assign-start-date"
            label="Start Date"
            type="date"
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              id="trainer-assign-duration"
              label="Duration (weeks)"
              type="number"
              value={assignForm.durationWeeks}
              onChange={(e) =>
                setAssignForm((f) => ({ ...f, durationWeeks: e.target.value }))
              }
            />
            <Input
              id="trainer-assign-days"
              label="Days / week"
              type="number"
              value={assignForm.daysPerWeek}
              onChange={(e) =>
                setAssignForm((f) => ({ ...f, daysPerWeek: e.target.value }))
              }
            />
          </div>
          <Select
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleAssign}>
              Assign
            </LoadingButton>
          </div>
        </div>
