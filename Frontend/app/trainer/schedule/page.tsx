...
export default function TrainerSchedulePage() {
    ...
    (myShiftRows ?? []).forEach((sh: any) => {
      ...
      if (!byDay.has(day)) {
        byDay.set(day, {
          ...
          type:
            sh.status === "missed" || sh.status === "swapped" ? "off" : "shift",
        });
      }
    });
    setShifts(
  ...
  useEffect(() => {
    loadSchedule().catch((err) =>
      toast.error("Failed to load schedule", getErrorMessage(err)),
    );
  }, []);
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      toast.success(
        "Updated",
        `Session marked as ${status.replace("_", " ")}.`,
      );
      await loadSchedule();
    } catch (err) {
  ...
  const handleCancel = async () => {
    ...
    try {
      await handleStatusUpdate(
        cancelModal,
        "cancelled",
        cancelReason.trim() || "Cancelled by trainer",
      );
      setCancelModal(null);
      setCancelReason("");
    } finally {
  ...
  const handleRequestOverride = async () => {
    ...
    try {
      ...
      toast.success(
        "Request Sent",
        "Your override request has been submitted to the manager.",
      );
  ...
  const upcoming = sessions.filter((s) =>
    ["booked", "confirmed"].includes(s.status),
  );
      ...
      <Card padding="lg">
        ...
        <div className="space-y-3">
          {upcoming.map((s) => (
            <div
              key={s.id}
              className="flex min-w-0 flex-col gap-3 bg-zinc-800/30 rounded-xl p-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              ...
              <div className="flex w-full min-w-0 flex-col gap-2 sm:w-auto sm:max-w-[min(100%,28rem)] sm:flex-row sm:flex-wrap sm:items-center sm:justify-end sm:gap-2">
                <span
                  className={`w-fit shrink-0 text-xs px-2 py-1 font-semibold capitalize rounded-full ${statusStyles[s.status]}`}>
                  {s.status}
                </span>
            ...
            {past.slice(0, 6).map((s) => (
              <div key={s.id} className="bg-zinc-800/20 rounded-xl p-3 gap-2">
                <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                  ...
                  <span
                    className={`w-fit shrink-0 self-start text-xs px-2 py-0.5 font-semibold capitalize rounded-full sm:self-center ${statusStyles[s.status]}`}>
                    {s.status.replace("_", " ")}
                  </span>
                </div>
                {s.status === "completed" && s.reviewRating != null && (
                  <div className="mt-2 pt-2 border-t border-zinc-700/40 text-xs text-zinc-500">
                    ...
                    {s.reviewComment ? (
                      <p className="mt-1 text-zinc-400">{s.reviewComment}</p>
                    ) : null}
                  </div>
                )}
              </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          <Select
            ...
            onChange={(e) =>
              setOverrideForm((f) => ({ ...f, type: e.target.value }))
            }
          />
          <Textarea
