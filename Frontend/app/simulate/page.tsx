  ...
  const refreshQr = useCallback(async () => {
    ...
    try {
      ...
      const initialLeft = Math.max(
        ...
        Math.min(
          30,
          Math.floor((new Date(otp.expiresAt).getTime() - Date.now()) / 1000),
        ),
      );
  ...
  const pollState = useCallback(async () => {
    ...
    try {
      ...
      if (
        best &&
        best.status !== "denied" &&
        ["active", "completed", "auto_closed"].includes(
          String(best.status ?? ""),
        )
      ) {
        ...
        if (age < 10000) {
          ...
          if (lastDoorEventRef.current !== eventKey) {
            ...
            if (doorCloseTimerRef.current)
              window.clearTimeout(doorCloseTimerRef.current);
            doorCloseTimerRef.current = window.setTimeout(
              () => setDoorOpen(false),
              3200,
            );
          }
      ...
      const status =
        typeof e?.status === "number" ? e.status : e?.response?.status;
      if (status === 429) {
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-zinc-400 mt-2 max-w-2xl text-sm md:text-base leading-relaxed">
          This page mimics real hardware: a time-limited door QR, animated
          entry, and a mock card network that auto-approves valid-length PANs.
          Open <span className="text-red-400 font-medium">Check-in</span> on
          your phone while logged in, allow the camera, and scan the code below.
        </p>
      </div>
      ...
      <div className="space-y-8">
        <Card padding="lg" className="space-y-6 bg-zinc-900/40 border-zinc-800">
          <div className="flex items-center justify-between gap-3">
            ...
            <LoadingButton
              ...
              variant="secondary"
              icon={RefreshCw}
              onClick={() => refreshQr().catch(() => undefined)}>
              Refresh QR
            </LoadingButton>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
            <div
              className={`relative mx-auto w-full max-w-md aspect-[4/5] rounded-2xl border-2 transition-all duration-500 overflow-hidden ${
                doorOpen
                  ? "border-emerald-500/70 shadow-[0_0_46px_rgba(16,185,129,0.32)]"
                  : "border-zinc-700"
              }`}
              ...
              <div className="absolute inset-x-0 bottom-4 z-10 flex justify-center">
                {doorOpen ? (
                  ...
                  <span className="text-zinc-300 text-xs uppercase tracking-wider bg-zinc-900/50 border border-zinc-700 rounded-full px-3 py-1">
                    Locked
                  </span>
                )}
              </div>
            ...
            <div className="w-full flex flex-col items-center xl:items-start gap-3">
              ...
              <p
                className={`font-mono text-xl ${countdownSec <= 8 ? "text-amber-300" : "text-zinc-200"}`}>
                {countdownText}
              </p>
            </div>
          </div>
        ...
        <Card padding="lg" className="space-y-5 bg-zinc-900/40 border-zinc-800">
          ...
          <p className="text-zinc-500 text-sm">
            Incoming checkouts are settled directly into the canonical
            `payments` ledger.
          </p>
          <div className="rounded-xl border border-zinc-700 bg-zinc-950/60 p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 text-emerald-300">
              <CheckCircle2 size={18} /> Processor online
            </div>
          </div>
          <div className="space-y-2">
            <p className="text-xs text-zinc-500">
              Recent settlements: {recentPayments.length}
            </p>
            {recentPayments.slice(0, 3).map((row) => (
              <div
                key={row.id}
                className="rounded-lg border border-zinc-800 px-3 py-2 text-xs text-zinc-300">
                {row.paymentMethod ?? "online"} • {row.status ?? "completed"} •
