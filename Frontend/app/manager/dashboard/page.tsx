...
export default function ManagerDashboard() {
  ...
  const [branchInsights, setBranchInsights] = useState<
    Array<{ title: string; description: string; impact: Impact; rec: string }>
  >([]);
  ...
  const [tasks, setTasks] = useState<
    Array<{ task: string; priority: Priority; due: string; assignee: string }>
  >([]);
  ...
  const briefingComplete =
    Boolean(latestAi?.content) &&
    streamedBriefing.length >= (latestAi?.content?.length ?? 0);
  ...
  const refresh = async () => {
    ...
    setBranchInsights([
      {
        title: "Visits Today",
        description: `${dash.todayVisits ?? 0} recorded check-ins today`,
        impact: "high",
        rec: "Optimize trainer staffing during peaks",
      },
      {
        title: "Open Issues",
        description: `${dash.openIssues ?? 0} unresolved incidents`,
        impact: dash.openIssues > 0 ? "medium" : "positive",
        rec: "Review equipment incident queue daily",
      },
      {
        title: "Revenue",
        description: `$${Number(summary.monthlyRevenue ?? 0).toLocaleString("en-US")} this month`,
        impact: "positive",
        rec: "Track conversion from trial to paid plans",
      },
    ]);
    setTasks(
  ...
  const kpis = [
    {
      label: "Total Members",
      value: String(dashboard?.activeMembers ?? 0),
      sub: "active",
      icon: Users,
    ...
    },
    {
      ...
      value: `$${Number(dashboard?.monthlyRevenue ?? 0).toLocaleString("en-US")}`,
      sub: "current month",
      icon: TrendingUp,
    ...
    {
      label: "Visits Today",
      value: String(dashboard?.todayVisits ?? 0),
      sub: "facility entries",
      icon: UserCheck,
      color: "from-purple-600 to-purple-700",
    },
    {
      label: "Open Issues",
      value: String(dashboard?.openIssues ?? 0),
      sub: "action items",
      icon: Star,
    ...
    <div className="space-y-8">
          ...
          series={[
            {
              name: "Visits",
              color: "#ef4444",
              values: analytics.activityOverview.map((p) =>
                Number(p.visits ?? 0),
              ),
            },
            {
      ...
      <Card padding="md">
              ...
              window.dispatchEvent(
                new CustomEvent("pw:ai-chat-prefill", {
                  detail: {
                    role: "manager",
                    message:
                      "Summarize key branch risks and actions for this week.",
                  },
                }),
              )
            }
        ...
        <p className="mt-2 text-xs text-zinc-500 leading-relaxed">
          Choose a briefing—full Markdown is generated in place with a live
          reveal. Follow up in chat anytime.
        </p>
        ...
        {(aiLoading || streamedBriefing) && (
          <div className="mt-4 rounded-xl border border-violet-500/20 bg-violet-950/25 px-4 py-3 max-h-[28rem] overflow-y-auto">
            {aiLoading && !streamedBriefing && (
              <p className="text-sm text-violet-200/90 animate-pulse">
                Generating briefing…
              </p>
            )}
            {streamedBriefing ? (
        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
              <Lightbulb size={18} className="text-yellow-400" /> Key Insights
            </h2>
            <Link
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
            {branchInsights.map((ins, i) => (
              <div key={`kpi-${i}`} className="bg-zinc-800/30 rounded-xl p-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${impactColor[ins.impact]}`}>
                    {ins.impact}
                  </span>
                </div>
            ...
            {latestAi && latestAi.insights.length > 0 && (
              <>
                <p className="text-[11px] text-zinc-500 uppercase tracking-wide pt-2">
                  Recommendations
                </p>
                {latestAi.insights.map((line, i) => (
                  <div
                    key={`ai-${i}`}
                    className="bg-violet-950/40 border border-violet-500/20 rounded-xl p-4">
                    <div className="flex justify-between mb-1">
                      <p className="text-violet-200 text-sm font-semibold">
                        Action {i + 1}
                      </p>
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${impactColor[(i === 0 ? "high" : i === 1 ? "medium" : "low") as Impact]}`}>
            ...
            {tasks.map((t, i) => (
              <div key={i} className="bg-zinc-800/30 rounded-xl p-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${priorityColor[t.priority]}`}>
                    {t.priority}
                  </span>
                </div>
