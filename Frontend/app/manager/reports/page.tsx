...
const REPORT_TYPES = [
  {
    ...
    label: "Membership",
    icon: Users,
    desc: "New registrations and subscriptions by plan",
  },
  {
    id: "revenue",
    label: "Revenue",
    icon: TrendingUp,
    desc: "Payments by method and plan",
  },
  {
    id: "attendance",
    label: "Attendance",
    icon: Activity,
    desc: "Daily visits and peak hours",
  },
  {
    id: "trainer",
    label: "Trainers",
    icon: Dumbbell,
    desc: "PT session stats per trainer",
  },
  {
    id: "equipment",
    label: "Equipment",
    icon: Wrench,
    desc: "Incidents by severity and equipment",
  },
  {
    id: "inventory",
    label: "Inventory",
    icon: Package,
    desc: "Stock position, low-stock risk, and movement",
  },
];
          ...
          {REPORT_TYPES.map((r) => (
            <div
              ...
              className={cn(
                ...
                selected === r.id ? reportTypeTileActive : reportTypeTileIdle,
              )}>
              <r.icon
                size={16}
                className={
                  selected === r.id ? "text-red-400 mb-1" : "text-zinc-500 mb-1"
                }
              />
      ...
      {reportData && (
        <div className="space-y-6">
            ...
            {[
              {
                ...
                value: `$${Number(reportData.monthlyRevenue ?? 0).toLocaleString("en-US")}`,
              },
              {
              ...
              },
              {
                ...
                value: reportData.visitsInRange ?? "—",
              },
              {
                label: "Open Incidents",
                value: reportData.openEquipmentIncidents ?? "—",
              },
            ].map((kpi) => (
              <Card
                key={kpi.label}
                padding="md"
                className={cn(reportSectionCard, "border-zinc-700/60")}>
          ...
          {reportData.type === "revenue" &&
            (reportData.totalRevenueInRange != null ||
              reportData.paymentCountInRange != null) && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Card
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    $
                    {Number(reportData.totalRevenueInRange ?? 0).toLocaleString(
                      "en-US",
                    )}
                  </p>
                </Card>
                <Card
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    {reportData.paymentCountInRange ?? "—"}
                  </p>
                </Card>
                <Card
                  padding="md"
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    $
                    {Number(
                      reportData.averagePaymentInRange ?? 0,
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className={reportTableHeadRow}>
                      ...
                      <th className={cn(reportTableCellHead, "text-right")}>
                        Transactions
                      </th>
                      <th className={cn(reportTableCellHead, "text-right")}>
                        Total
                  ...
                  <tbody>
                    {(reportData.byMethod ?? []).map((r: any, i: number) => (
                      <tr key={i} className={reportTableBodyRow}>
                        ...
                        <td
                          className={cn(
                            reportTableCell,
                            "text-right text-white font-medium",
                          )}>
                        ...
                        <td
                          ...
                          {r.pctOfTotalRevenue != null
                            ? `${Number(r.pctOfTotalRevenue).toFixed(1)}%`
                            : "—"}
                        </td>
                      </tr>
                    ))}
                ...
                <>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {(reportData.byPlan ?? []).map((r: any, i: number) => (
                          <tr key={i} className={reportTableBodyRow}>
                            <td
                              className={cn(reportTableCell, "text-zinc-300")}>
                              {r.planName ?? "Unknown"}
                            </td>
                            <td
                              className={cn(
          ...
          {reportData.type === "membership" && (
            <Card padding="lg" className={reportSectionCard}>
              <p className="text-zinc-400 text-sm mb-4">
                ...
                {reportData.subscriptionsCreatedInRange != null && (
                  <>
                    ...
                    <span className="text-white font-bold">
                      {reportData.subscriptionsCreatedInRange}
                    </span>
                  </>
                )}
                {reportData.activeSubscriptionsTotal != null && (
                  <>
                    ...
                    <span className="text-white font-bold">
                      {reportData.activeSubscriptionsTotal}
                    </span>
                  </>
                )}
              </p>
              ...
              {reportData.byStatus && (
                <>
                  <h4 className="text-zinc-300 font-medium mb-2 mt-4">
                    Subscription status (created in range)
                  </h4>
                  <div className="flex flex-wrap gap-3">
                    {(reportData.byStatus ?? []).map((r: any, i: number) => (
                      <div
                        key={i}
                        className="bg-zinc-800/60 border border-zinc-700/80 rounded-xl px-4 py-2 text-center">
                        ...
                        {r.pctOfCreated != null && (
                          <p className="text-zinc-500 text-xs">
                            {Number(r.pctOfCreated).toFixed(1)}% of created
                          </p>
                        )}
                      </div>
                    ))}
            ...
            <Card padding="lg" className={reportSectionCard}>
              ...
              {reportData.avgVisitsPerDayInRange != null && (
                <p className="text-zinc-400 text-sm mb-4">
                  ...
                  <span className="text-white font-semibold">
                    {reportData.avgVisitsPerDayInRange}
                  </span>
                </p>
              )}
              {reportData.byHour && (
                <div className="flex items-end gap-1 h-20 mb-6">
                  {Array.from({ length: 24 }, (_, h) => {
                    const found = (reportData.byHour ?? []).find(
                      (r: any) => Number(r.hour) === h,
                    );
                    const max = Math.max(
                      ...(reportData.byHour ?? []).map((r: any) =>
                        Number(r.count),
                      ),
                      1,
                    );
                    ...
                    return (
                      <div
                        ...
                        />
                        {h % 6 === 0 && (
                          <span className="text-zinc-500 text-xs">{h}h</span>
                        )}
                      </div>
                    );
                  })}
              ...
              <div className="max-h-48 overflow-y-auto overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(reportData.daily ?? []).map((r: any, i: number) => (
                      <tr key={i} className={reportTableBodyRow}>
                        <td className={cn(reportTableCell, "text-zinc-300")}>
                          {r.date}
                        </td>
                        <td
                          className={cn(
                        ...
                        <td
                          ...
                          {r.avgDurationMin ? `${r.avgDurationMin} min` : "—"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {reportData.type === "trainer" && reportData.trainerStats && (
            <Card padding="lg" className={reportSectionCard}>
              ...
              {reportData.ptSessionsInRange != null && (
                <p className="text-zinc-400 text-sm mb-4">
                  PT sessions in range:{" "}
                  <span className="text-white font-semibold">
                    {reportData.ptSessionsInRange}
                  </span>
                </p>
              )}
              <div className="overflow-x-auto">
                        ...
                        const pct =
                          r.completionRatePct != null
                            ...
                            : r.total > 0
                              ? String(
                                  Math.round(
                                    (Number(r.completed) / Number(r.total)) *
                                      100,
                                  ),
                                )
                              : "0";
              ...
              {reportData.byEquipment && (
                <div className="overflow-x-auto">
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(reportData.byEquipment ?? []).map(
                        (r: any, i: number) => (
                          <tr key={i} className={reportTableBodyRow}>
                            <td
                              className={cn(reportTableCell, "text-zinc-300")}>
                              {r.equipmentName ?? "Unknown"}
                            </td>
                            <td
                              className={cn(
                                reportTableCell,
            ...
            <Card padding="lg" className={reportSectionCard}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <Card
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    {reportData.totalTransactionsInRange ?? "—"}
                  </p>
                </Card>
                <Card
                  padding="md"
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    {reportData.totalQtyMovedInRange ?? "—"}
                  </p>
                </Card>
                <Card
                  padding="md"
                  ...
                  <p className="text-white text-xl font-bold mt-1">
                    {reportData.netStockChangeInRange ?? "—"}
                  </p>
                </Card>
              </div>
              ...
              {reportData.topMovementItems && (
                <>
                  <h4 className="text-zinc-300 font-medium mb-2 mt-6">
                    Top movement items
                  </h4>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {(reportData.topMovementItems ?? []).map(
                          (r: any, i: number) => (
                            <tr key={i} className={reportTableBodyRow}>
                              <td
                                className={cn(
