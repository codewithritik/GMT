...
export default function ManagerInventoryPage() {
  ...
  useEffect(() => {
    loadData().catch((err) =>
      toast.error("Failed to load inventory", getErrorMessage(err)),
    );
  }, []);

  const lowStock = useMemo(
    () => items.filter((i) => i.qty < i.threshold),
    [items],
  );
  ...
  const handleTransaction = async () => {
    ...
    try {
      ...
      toast.success(
        "Recorded",
        `${form.txnType} of ${form.qty} units recorded`,
      );
      setTxOpen(false);
    ...
    <div className="space-y-8">
            ...
            <div
              ...
              <div>
                ...
                <p className="text-zinc-500 text-xs">
                  {t.txnType} · by {t.recorderName ?? "team"}
                </p>
              </div>
              <div className="text-right">
                <p
                  className={`text-sm font-semibold ${t.qtyChange > 0 ? "text-green-400" : "text-red-400"}`}>
                  {t.qtyChange > 0 ? "+" : ""}
                  {t.qtyChange}
                </p>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          />
          <Select
            id="mgr-inventory-type"
            label="Type"
            options={TX_OPTIONS}
            value={form.txnType}
            onChange={(e) =>
              setForm((f) => ({ ...f, txnType: e.target.value as TxType }))
            }
          />
          <Input
            id="mgr-inventory-qty"
            label="Quantity"
            type="number"
            value={form.qty}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <LoadingButton variant="secondary" onClick={() => setTxOpen(false)}>
              Cancel
            </LoadingButton>
            <LoadingButton loading={loading} onClick={handleTransaction}>
              Record
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          />
          <Input
            id="mgr-item-threshold"
            label="Reorder Threshold"
            type="number"
            value={newItem.threshold}
            onChange={(e) =>
              setNewItem((n) => ({ ...n, threshold: e.target.value }))
            }
          />
          <div className="flex justify-end gap-3 pt-2">
            <LoadingButton
            ...
            <LoadingButton loading={loading} onClick={handleAddItem}>
              Add Item
            </LoadingButton>
          </div>
        </div>
