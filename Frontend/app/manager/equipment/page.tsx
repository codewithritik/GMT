...
export default function ManagerEquipmentPage() {
  ...
  const handleAddEquipment = async () => {
    ...
    try {
      ...
      toast.success(
        "Equipment Added",
        `${addEquipForm.name} has been added to inventory`,
      );
  ...
  const handleReportIssue = async () => {
    if (!reportForm.equipmentId || !reportForm.issue.trim()) {
      toast.error(
        "Validation Error",
        "Select equipment and describe the issue",
      );
      return;
    }
    setLoading(true);
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      await opsAPI.updateEquipment(selectedReport.equipmentId, {
        status: "operational",
      });
      await loadData();
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: "Out of Order",
            value: equipment.filter((e) => e.status === "out_of_order").length,
            icon: AlertTriangle,
            color: "text-red-400",
          },
            ...
            <div
              ...
              <div>
                ...
                <p className="text-zinc-500 text-sm capitalize">
                  {e.category.replace("_", " ")} · Zone: {e.zone} · Qty:{" "}
                  {e.quantity}
                </p>
              </div>
              <span
      ...
      <Card padding="lg">
        <h2 className="text-lg font-semibold text-white mb-4">
          Open Issue Reports ({openReports.length})
        </h2>
        <div className="space-y-3">
          {openReports.length === 0 && (
            <p className="text-zinc-500 text-sm">No open issues.</p>
          )}
          {openReports.map((r) => (
            <div
              key={r.id}
              ...
              <div>
                <p className="text-white font-semibold">
                  {equipmentNameById.get(r.equipmentId) ?? "Equipment"}
                </p>
      ...
      <Modal
        ...
        <div className="space-y-4">
          <Input
            ...
            onChange={(e) =>
              setAddEquipForm((f) => ({ ...f, name: e.target.value }))
            }
          ...
          <Select
            ...
            onChange={(e) =>
              setAddEquipForm((f) => ({
                ...f,
                category: e.target.value as typeof addEquipForm.category,
              }))
            }
          />
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                setAddEquipForm((f) => ({ ...f, zoneLabel: e.target.value }))
              }
            ...
            <Input
              ...
              onChange={(e) =>
                setAddEquipForm((f) => ({ ...f, quantity: e.target.value }))
              }
              min="1"
            />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleAddEquipment}>
              Add Equipment
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          <Select
            ...
            onChange={(e) =>
              setReportForm((f) => ({ ...f, equipmentId: e.target.value }))
            }
          ...
          <Select
            ...
            onChange={(e) =>
              setReportForm((f) => ({
                ...f,
                severity: e.target.value as typeof reportForm.severity,
              }))
            }
          />
          <Textarea
            ...
            onChange={(e) =>
              setReportForm((f) => ({ ...f, issue: e.target.value }))
            }
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleReportIssue}>
              Report Issue
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        title="Resolve Issue"
        description={selectedReport ? `Resolving: ${selectedReport.issue}` : ""}
        size="md">
        <div className="space-y-4">
          <Textarea
            id="resolve-notes"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleResolve}>
              Mark Resolved
            </LoadingButton>
          </div>
        </div>
