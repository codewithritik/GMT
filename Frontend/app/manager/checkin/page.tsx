...
export default function ManagerCheckinPage() {
  ...
  useEffect(() => {
    refresh().catch((err) =>
      toast.error("Failed to load check-in data", getErrorMessage(err)),
    );
  }, []);
  useRealtimePolling(() => {
  ...
  const inCount = useMemo(
    () =>
      log.filter((l) => l.type === "in" && l.subscription !== "expired").length,
    [log],
  );
  const denied = useMemo(
    () => log.filter((l) => l.subscription === "expired").length,
    [log],
  );
  ...
  return (
    <div className="space-y-10">
      ...
      <PageHeader
        ...
        subtitle="Monitor entries and exits"
      />
        ...
        <div className="w-full bg-zinc-800 rounded-full h-3">
          <div
            className={`h-3 rounded-full transition-all ${
              inCount / branchCapacity > 0.8
                ? "bg-red-500"
                : inCount / branchCapacity > 0.6
                  ? "bg-yellow-500"
                  : "bg-green-500"
            }`}
            style={{
              width: `${Math.min(100, (inCount / branchCapacity) * 100)}%`,
            }}
          />
        </div>
      </Card>
        ...
        <div className="space-y-2 max-h-[24rem] overflow-y-auto">
          {filtered.map((l, i) => (
            <div
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${accessColor[l.access]}`}>
                  {l.access}
                </span>
                {l.subscription === "expired" && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold text-red-400 bg-red-500/20">
                    expired
                  </span>
                )}
