...
export default function ManagerClosuresPage() {
  ...
  useEffect(() => {
    loadClosures().catch((err) =>
      toast.error("Failed to load closures", getErrorMessage(err)),
    );
  }, []);
    ...
    <div className="space-y-8">
              ...
              {closures.map((c) => (
                <tr
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-semibold ${c.emergency ? "bg-red-500/20 text-red-400" : "bg-zinc-500/20 text-zinc-400"}`}>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          />
          <label
            htmlFor="closures-emergency"
            className="flex items-center gap-2 cursor-pointer">
            <input
              id="closures-emergency"
              type="checkbox"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleAdd}>
              Add Closure
            </LoadingButton>
          </div>
        </div>
