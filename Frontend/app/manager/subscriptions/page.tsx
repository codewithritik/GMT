...
export default function ManagerSubscriptionsPage() {
  ...
  useEffect(() => {
    loadPlans().catch((err) =>
      toast.error("Failed to load subscriptions", getErrorMessage(err)),
    );
  }, []);
                ...
                {plans.map((p) => (
                  <tr
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.active ? "bg-emerald-500/20 text-emerald-400" : "bg-zinc-500/20 text-zinc-400"}`}>
          ...
          <Card padding="none">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-700">
                    <th className="text-left text-xs font-semibold text-zinc-400 uppercase px-5 py-4">
                      Member
                    </th>
                    <th className="text-left text-xs font-semibold text-zinc-400 uppercase px-5 py-4">
                      Plan
                ...
                <tbody>
                  {filteredSubs.map((s) => (
                    <tr
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${statusStyle[s.status] ?? "bg-zinc-500/20 text-zinc-400"}`}>
                          {s.status.replace("_", " ")}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-3 text-zinc-300 text-sm">
                        ${s.pricePaid.toLocaleString("en-US")}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="space-y-4">
          ...
          {!editing && (
            <Select
              ...
              value={form.planType}
              onChange={(e) =>
                setForm((f) => ({ ...f, planType: e.target.value }))
              }
            />
          )}
          <Input
            id="subscriptions-price"
            label="Price (USD)"
            type="number"
            value={form.price}
