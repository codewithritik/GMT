...
export default function ManagerMembersPage() {
  ...
  const loadMembers = async () => {
    ...
    (visits ?? []).forEach((v: any) =>
      byPerson.set(v.personId, (byPerson.get(v.personId) ?? 0) + 1),
    );
    ...
    try {
      if (editingMember) {
        await opsAPI.updateUser(editingMember.id, {
          ...
          ...(formData.trainerId
            ? { assignedTrainerId: formData.trainerId }
            : {}),
        });
        toast.success(
          "Member Updated",
        ...
        toast.success(
          "Member Added",
          `${formData.name} has been added. Default password: TempPass123!`,
        );
      }
      await loadMembers();
      setModalOpen(false);
            ...
            <tbody>
              {filtered.map((m, i) => (
                <tr
                  ...
                  <td className="px-5 py-4 text-center text-zinc-300 text-sm font-semibold">
                    {m.checkins}
                  </td>
