...
export default function ManagerInsightsPage() {
  ...
  const streamDone =
    Boolean(aiResult?.content) &&
    streamedContent.length >= (aiResult?.content?.length ?? 0);
  ...
  const kpis = [
    {
      ...
      value: String(Math.round((summary?.visitsLast30Days ?? 0) / 30)),
      delta: "from live visit logs",
      good: true,
    },
    {
      label: "Monthly Revenue",
      value: `$${Number(summary?.monthlyRevenue ?? 0).toLocaleString("en-US")}`,
      delta: "from payment records",
      good: true,
    },
    {
    ...
    },
    {
      ...
      value: String(summary?.openEquipmentIncidents ?? 0),
  ...
  const occupancyTrend = useMemo(
    () => analytics?.occupancyTrend ?? [],
    [analytics],
  );
  const occupancyByHour = useMemo(
    () => analytics?.avgHourlyOccupancy ?? [],
    [analytics],
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((k) => (
          <Card
            ...
            <p
              className={`text-xs mt-1 flex items-center gap-1 ${k.good ? "text-green-400" : "text-red-400"}`}>
              <ArrowUp size={10} className={k.good ? "" : "rotate-180"} />{" "}
              {k.delta}
            </p>
          </Card>
        ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            series={[
              {
                name: "Visits",
                color: "#ef4444",
                values: occupancyTrend.map((p: any) => Number(p.value ?? 0)),
              },
            ]}
            height={210}
          />
        ...
        <Card padding="lg">
          ...
          <ThemedLineChart
            ...
            series={[
              {
                name: "Avg check-ins/hour",
                color: "#a855f7",
                values: occupancyByHour.map((p: any) => Number(p.value ?? 0)),
              },
            ]}
            height={210}
          />
            ...
            series={[
              {
                name: "Revenue (USD)",
                color: "#22c55e",
                values: revenueTrend.map((p: any) => Number(p.value ?? 0)),
              },
            ]}
            height={210}
          />
            ...
            series={[
              {
                name: "Visits",
                color: "#ef4444",
                values: activityOverview.map((p: any) => Number(p.visits ?? 0)),
              },
              {
                name: "Workouts",
                color: "#3b82f6",
                values: activityOverview.map((p: any) =>
                  Number(p.workouts ?? 0),
                ),
              },
              {
      ...
      <Card padding="md">
              ...
              window.dispatchEvent(
                new CustomEvent("pw:ai-chat-prefill", {
                  detail: {
                    role: "manager",
                    message:
                      "I need a deeper dive on branch operations—ask me one clarifying question, then advise.",
                  },
                }),
              )
            }
        ...
        <p className="mt-1 text-xs text-zinc-500">
          Run a preset analysis—the full response streams in below. Use chat for
          follow-ups.
        </p>
        ...
        <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
          {(!aiResult?.insights || aiResult.insights.length === 0) && (
            <p className="text-zinc-500 text-sm">
              Choose an action above to populate parsed action bullets here (in
              addition to the full briefing).
            </p>
          )}
          {(aiResult?.insights ?? []).map((line, i) => (
            <Card
              key={i}
              padding="md"
              className="hover:border-zinc-700/50 transition-colors border-violet-500/15 bg-violet-950/20">
              <div className="flex items-start justify-between mb-2">
                <p className="text-white font-semibold">
                  Recommendation {i + 1}
                </p>
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${impactColor[i === 0 ? "high" : i === 1 ? "medium" : "low"]}`}>
