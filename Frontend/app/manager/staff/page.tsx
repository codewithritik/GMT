...
export default function ManagerStaffPage() {
  ...
  const loadData = async () => {
    ...
    (members ?? []).forEach((m: any) => {
      if (m.assignedTrainerId)
        activeByTrainer.set(
          m.assignedTrainerId,
          (activeByTrainer.get(m.assignedTrainerId) ?? 0) + 1,
        );
    });
    ...
    setStaff(
      (trainers ?? []).map((t: any) => {
        ...
        return {
          ...
          todayShift: todayShiftInfo
            ? `${todayShiftInfo.shiftType.replace("_", " ")} (${todayShiftInfo.status})`
            : null,
          shiftStatus: (todayShiftInfo?.status ??
            "none") as StaffRow["shiftStatus"],
  ...
  const onShift = useMemo(
    () => staff.filter((s) => s.checkedIn).length,
    [staff],
  );
  ...
  const handleAddStaff = async () => {
    ...
    try {
      ...
      toast.success(
        "Trainer added",
        `${addForm.name} has been added. Temp password: TempPass123!`,
      );
      setAddOpen(false);
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      toast.success(
        "Shift Scheduled",
        `${selectedStaff.name} assigned ${scheduleForm.shiftType} shift on ${scheduleForm.shiftDate}`,
      );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {staff.map((s) => (
          <Card
            key={s.id}
            padding="md"
            className="flex flex-col gap-4 hover:border-zinc-700/50 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <span
                className={`ml-auto text-[10px] px-2 py-0.5 rounded-full font-semibold ${s.checkedIn ? "bg-green-500/20 text-green-400" : "bg-zinc-700 text-zinc-500"}`}>
                {s.checkedIn ? (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {s.todayShift ? (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${shiftStatusColor(s.shiftStatus)}`}>
                    {s.todayShift}
                  </span>
                ) : (
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            onChange={(e) =>
              setAddForm((f) => ({ ...f, employeeCode: e.target.value }))
            }
            placeholder="EMP001"
          />
          <Input
            ...
            onChange={(e) =>
              setAddForm((f) => ({ ...f, hireDate: e.target.value }))
            }
          />
          <p className="text-zinc-500 text-xs">
            Default password:{" "}
            <code className="text-zinc-300">TempPass123!</code> — they should
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleAddStaff}>
              Add Staff
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        title="Assign Shift"
        description={
          selectedStaff ? `Scheduling shift for ${selectedStaff.name}` : ""
        }
        size="md">
        <div className="space-y-4">
          <Input
            ...
            onChange={(e) =>
              setScheduleForm((f) => ({ ...f, shiftDate: e.target.value }))
            }
            required
          />
          <Select
            ...
            onChange={(e) =>
              setScheduleForm((f) => ({
                ...f,
                shiftType: e.target.value as typeof scheduleForm.shiftType,
              }))
            }
          />
          <Input
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleSchedule}>
              Assign Shift
            </LoadingButton>
          </div>
        </div>
