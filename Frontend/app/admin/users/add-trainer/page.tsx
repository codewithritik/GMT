...
function TrainerDetailsSection({
  register,
  errors,
}: TrainerDetailsSectionProps) {
  return (
    <>
      ...
      .then((rows) => {
        const options = (rows ?? [])
          ...
          .map((plan: any) => ({
            ...
            label:
              plan.isActive === false ? `${plan.name} (inactive)` : plan.name,
          }));
        setPlanOptions(options);
      })
      .catch((err) =>
        toast.error("Failed to load membership plans", getErrorMessage(err)),
      );
  }, [toast]);
