...
export default function AdminUsersPage() {
  ...
  const loadUsers = async () => {
    ...
    const mapped: User[] = (data ?? []).map((u: any) => ({
      ...
      joined: u.joinDate
        ...
        : u.accountCreatedAt
          ? String(u.accountCreatedAt).slice(0, 10)
          : null,
      isActive: u.isActive,
    }));
    setUsers(mapped);
  ...
  const filtered = users.filter((u) => {
    ...
    return matchS && matchR && matchStatus;
  });

  const openAdd = (role: Role) => {
    if (role === ROLE_OPTIONS.trainer) {
      router.push("/admin/users/add-trainer");
    } else {
  ...
  const handleDeactivate = async () => {
    ...
    try {
      await opsAPI.updateUser(deactivateTarget.id, {
        isActive: false,
        ...(deactivateTarget.role === "member"
          ? { memberStatus: "suspended" }
          : {}),
      });
      toast.success(
        "User Deactivated",
        `${deactivateTarget.name} has been deactivated and suspended.`,
      );
    ...
    <div className="space-y-8">
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <LoadingButton
              icon={Plus}
              onClick={() => openAdd(ROLE_OPTIONS.member as Role)}
              size="md">
              Add User
            </LoadingButton>
            <LoadingButton
              icon={Plus}
              onClick={() => openAdd(ROLE_OPTIONS.trainer as Role)}
              size="md">
              Add Trainer
            </LoadingButton>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {(["admin", "manager", "trainer", "member"] as Role[]).map((r) => (
          <Card
            key={r}
            padding="md"
            className="text-center hover:border-zinc-700/50 transition-colors">
            <p className="text-2xl font-bold text-white">{roleCounts[r]}</p>
            <p
              className={`text-xs font-semibold capitalize mt-1 ${roleColor[r].split(" ")[0]}`}>
              {r}s
            </p>
          </Card>
        ))}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {(["all", "admin", "manager", "trainer", "member"] as const).map(
            (r) => (
              <button
                key={r}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {/* <SearchInput
                    ...
                    /> */}
        <div className="flex gap-2 flex-wrap">
          {(["all", "active", "inactive", "suspended"] as const).map((r) => (
            <button
              key={r}
              onClick={() => setStatusFilter(r as UserStatus | "all")}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-all ${
                statusFilter === r
                  ? "bg-red-600 text-white border border-red-500"
                  : "bg-zinc-900/50 border border-zinc-800 text-zinc-400 hover:bg-zinc-800/50 hover:text-white"
      ...
      <Card padding="none">
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((u, i) => (
                <tr
                  key={u.id}
                  className={`border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors ${i === filtered.length - 1 ? "border-none" : ""}`}>
                  ...
                  <td className="px-5 py-4 text-center">
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-semibold ${roleColor[u.role]}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-center">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center text-zinc-500 text-sm">
            {pageLoading ? "Loading users..." : "No users found."}
          </div>
        )}
      </Card>
      ...
      <Modal
        ...
        title="Edit User"
        description={editingUser ? `Editing ${editingUser.name}` : undefined}
        size="lg">
      ...
      <Modal
        ...
        description={`Are you sure you want to deactivate ${deactivateTarget?.name}? They will be suspended and will no longer be able to log in.`}
        size="sm">
        <div className="flex justify-end gap-3 pt-2">
          <LoadingButton
            variant="secondary"
