...
export default function AdminUsersNewPage() {
  ...
  useEffect(() => {
    Promise.all([opsAPI.plans({ includeInactive: true }), opsAPI.trainers()])
      .then(([plans, trainers]) => {
        const options = (plans ?? [])
        ...
        setTrainerOptions(
          (trainers ?? [])
            .filter(
              (t: { id?: string; fullName?: string }) => t?.id && t?.fullName,
            )
      ...
      .catch((err) =>
        toast.error("Failed to load form options", getErrorMessage(err)),
      );
  }, [toast]);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (data.role !== "member") {
        toast.error(
          "Invalid role",
          "This page only supports creating members.",
        );
        return;
      }
