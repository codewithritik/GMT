...
export default function AdminAlertsPage() {
  ...
  const loadAlerts = async () => {
    ...
    const auditAlerts: Alert[] = (audits as any[])
      ...
      .map((r) => ({
        ...
        message:
          [r.detail, r.actorLabel].filter(Boolean).join(" — ") || "Audit event",
  ...
  useEffect(() => {
    loadAlerts().catch((err) =>
      toast.error("Failed to load alerts", getErrorMessage(err)),
    );
  }, []);
  ...
  const acknowledge = async (a: Alert) => {
    if (a.source === "audit") {
      toast.info(
        "Audit entry",
        "This is a read-only audit log line — nothing to acknowledge.",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        "Acknowledged",
        "Equipment alert acknowledged and marked in-progress.",
      );
      await loadAlerts();
    } catch (err) {
  ...
  const resolve = async (a: Alert) => {
    if (a.source === "audit") {
      toast.info(
        "Audit entry",
        "Resolve equipment issues from the list above; audit rows are informational only.",
      );
      return;
    }
            ...
            <Card
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 min-w-0 flex-1">
                ...
                <div className="flex gap-2 flex-wrap shrink-0">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-semibold border ${priorityColor[a.priority]}`}>
                    {a.priority}
                  </span>
                  <span
