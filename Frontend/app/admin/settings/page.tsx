...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    opsAPI
      ...
      .then((rows: any[]) => {
        ...
        setBackupFreq(
          m.get("db_backup_frequency") ?? m.get("backup_frequency") ?? "daily",
        );
      })
      .catch((err) =>
  ...
  const payload = useMemo(() => {
    ...
    for (const k of KEYS) {
      if (vals[k] != null && String(vals[k]).trim() !== "")
        o[k] = String(vals[k]).trim();
    }
    return o;
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <Card padding="lg" className="space-y-6">
        ...
        {n(
          "branch_capacity",
          "Max people in facility (display / capacity bar)",
          "Used on check-in dashboards.",
        )}
        {n(
          "checkin_qr_ttl_seconds",
          "Door QR time-to-live (seconds)",
          "15–600; simulator and hardware challenges.",
        )}
        {n(
          "checkin_scan_max_retries",
          "Max failed scan attempts per session",
          "Client hint; enforce in future rate limiter.",
        )}
      </Card>
      ...
      <Card padding="lg" className="space-y-6">
        ...
        {n(
          "payment_failure_max_retries",
          "Payment retry budget",
          "For billing workers / future automation.",
        )}
      </Card>
      ...
      <Card padding="lg" className="space-y-6">
        ...
        {n(
          ...
          "Applied on next failed login.",
        )}
        {n("login_failure_lock_minutes", "Account lock duration (minutes)")}
        {n(
          "session_idle_timeout_minutes",
          "Idle session hint (minutes)",
          "Documented for clients; refresh TTL is JWT-driven.",
        )}
      </Card>
      ...
      <Card padding="lg" className="space-y-6">
        ...
        {n(
          "ai_chat_rate_limit_per_minute",
          "AI chat requests / user / minute",
          "Documented for gateway tuning.",
        )}
        ...
        {[
          {
            ...
            sub: "Operational emails",
            value: emailN,
            set: () => setEmailN(!emailN),
          },
          {
            label: "SMS notifications",
            sub: "High-priority SMS",
            value: smsN,
            set: () => setSmsN(!smsN),
          },
        ].map((x) => (
