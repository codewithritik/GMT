...
export default function AdminPromotionsPage() {
  ...
  const loadPromos = () =>
    opsAPI
      ...
      .catch((err) =>
        toast.error("Failed to load promotions", getErrorMessage(err)),
      )
  ...
  const handleSave = async () => {
    if (
      ...
      toast.error(
        "Validation Error",
        "Code, name, discount, and valid-from date are required",
      );
      return;
    }
    setLoading(true);
  ...
  const formatDiscount = (p: Promotion) =>
    p.discountType === "percentage"
      ...
      : `$${Number(p.discountValue).toLocaleString("en-US")}`;
              ...
              {pageLoading ? (
                ...
                promos.map((p) => (
                  <tr
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.isActive ? "bg-emerald-500/20 text-emerald-400" : "bg-zinc-500/20 text-zinc-400"}`}>
