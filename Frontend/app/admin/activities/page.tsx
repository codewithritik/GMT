...
export default function AdminActivitiesPage() {
  ...
  useEffect(() => {
    opsAPI
      ...
      .then((rows: any[]) => {
        setEvents(
          (rows ?? []).map((r) => ({
            ...
            detail:
              [r.entityType, r.entityId, r.detail]
                .filter(Boolean)
                .join(" · ") || "—",
          })),
        );
      ...
      .catch((err) =>
        toast.error("Failed to load activity log", getErrorMessage(err)),
      );
  }, []);
  ...
  const filters: Array<EventType | "all"> = [
    "all",
    "member",
    "payment",
    "system",
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle="Append-only audit trail (check-ins, payments, config, broadcasts, …)"
      />
      ...
      <div className="space-y-3 max-h-[28rem] overflow-y-auto pr-1">
        {filtered.map((e) => (
          <Card
            ...
            <span className="text-zinc-600 text-xs flex-shrink-0 sm:self-end">
        ...
        {filtered.length === 0 && (
          <p className="text-center py-8 text-zinc-600">
            No events found. Run DB migration if the audit table is missing.
          </p>
        )}
      </div>
    </div>
