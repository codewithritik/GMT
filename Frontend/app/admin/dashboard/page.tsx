...
export default function AdminDashboard() {
  ...
  const [alerts, setAlerts] = useState<
    { type: AlertType; message: string; priority: string }[]
  >([]);
  const [activities, setActivities] = useState<
    Array<{ action: string; user: string; time: string; type: string }>
  >([]);
  const [plans, setPlans] = useState<
    Array<{ name: string; price: string; members: number }>
  >([]);
  ...
  const refresh = async () => {
    ...
    setAlerts(
      criticalOpen.slice(0, 3).map((e: any) => ({
    ...
    setPlans(
      (subscriptions ?? []).slice(0, 6).map((p: any) => ({
        name: p.name,
    ...
    <div className="space-y-8">
      ...
      {alerts.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-zinc-400 flex items-center gap-2">
            <AlertTriangle size={14} className="text-yellow-500" /> System
            Alerts
          </h2>
          {alerts.map((a, i) => (
            <div
              key={i}
              className={`flex items-center justify-between p-4 rounded-xl border ${alertColor[a.type]}`}>
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-current" />
                <span className="text-sm font-medium">{a.message}</span>
              </div>
              <span className="text-xs px-2 py-1 rounded-full bg-current/20 font-semibold capitalize">
                {a.priority}
              </span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            {activities.map((a, i) => (
              <div
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-9 h-9 rounded-lg ${activityColor[a.type]} flex items-center justify-center`}>
        ...
        <Card>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
              <ShieldCheck size={18} className="text-green-500" /> Plans
            </h2>
            <Link
              href="/admin/plans"
              className="text-sm text-red-500 hover:text-red-400">
