...
export default function AdminCheckinPage() {
  ...
  useEffect(() => {
    refresh().catch((err) =>
      toast.error("Failed to load visits", getErrorMessage(err)),
    );
  }, []);
  useRealtimePolling(() => {
  ...
  const inCount = useMemo(
    () => log.filter((l) => l.type === "in" && l.granted).length,
    [log],
  );
  ...
  return (
    <div className="space-y-10">
      <DoorQrCheckIn
        title="Check-in"
        subtitle="Scan the simulator door QR as admin (no subscription gate)."
        showCapacity={false}
      />

      <PageHeader
        title="Access control"
        subtitle="Full access log for the branch"
      />
      ...
      <Card padding="md">
        ...
        <div className="w-full bg-zinc-800 rounded-full h-3">
          <div
            className={`h-3 rounded-full transition-all ${inCount / 80 > 0.8 ? "bg-red-500" : inCount / 80 > 0.6 ? "bg-yellow-500" : "bg-green-500"}`}
      ...
      <Card padding="lg">
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {filtered.map((l, i) => (
            <div
              key={i}
              className={`flex items-center justify-between rounded-xl p-3 ${!l.granted ? "bg-red-950/30 border border-red-900/30" : "bg-zinc-800/30"}`}>
              <div className="flex items-center gap-3">
                {l.type === "in" ? (
                  <CheckCircle2
                    size={16}
                    className={l.granted ? "text-green-400" : "text-red-400"}
                  />
                ) : (
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${accessStyle[l.access]}`}>
                  {l.access}
                </span>
                {!l.granted && (
