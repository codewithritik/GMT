...
export default function AdminIdVerificationPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      await authAPI.adminVerifyId(
        selected.id,
        action,
        action === "rejected" ? note : undefined,
      );
      toast.success(
        action === "approved" ? "ID Approved" : "ID Rejected",
        `Verification updated for ${selected.fullName}`,
      );
      setVerifyOpen(false);
      setSelected(null);
      fetchSubmissions();
  ...
  const pending = submissions.filter(
    (s) => s.idVerificationStatus === "pending" || !s.idVerificationStatus,
  );
  const verified = submissions.filter(
    (s) => s.idVerificationStatus === "approved",
  );
  const rejected = submissions.filter(
    (s) => s.idVerificationStatus === "rejected",
  );
  ...
  return (
    <div className="space-y-8">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-700">
                  <th className="text-left text-xs font-semibold text-zinc-400 uppercase px-6 py-4">
                    Member
                  </th>
                  <th className="text-left text-xs font-semibold text-zinc-400 uppercase px-6 py-4">
                    Code
              ...
              <tbody>
                {submissions.map((s) => (
                  <tr
                    ...
                    <td className="px-6 py-4 text-zinc-400">
                      {s.idSubmittedAt
                        ? new Date(s.idSubmittedAt).toLocaleDateString()
                        : "—"}
                    </td>
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                          s.idVerificationStatus === "approved"
                            ? "bg-emerald-500/20 text-emerald-400"
                            : s.idVerificationStatus === "rejected"
      ...
      <Modal
        ...
        title="ID Documents"
        description={docMember ? docMember.fullName : ""}
        size="lg">
        <div className="space-y-4">
              ...
              <div>
                ...
                {docFrontUrl ? (
                  <img
                    src={docFrontUrl}
                    alt="Document front"
                    className="w-full rounded-xl border border-zinc-700 bg-zinc-900 object-contain max-h-80"
                  />
                ) : (
                  <div className="w-full h-40 rounded-xl border border-zinc-700 bg-zinc-800/50 flex items-center justify-center text-zinc-500 text-sm">
                    Failed to load
                  </div>
                )}
              </div>
          ...
          <div className="flex justify-end pt-2">
            <LoadingButton variant="secondary" onClick={closeDocModal}>
              Close
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        ...
        title={action === "approved" ? "Approve ID" : "Reject ID"}
        description={selected ? `Verifying ${selected.fullName}` : ""}
        size="sm">
        <div className="space-y-4">
          {action === "rejected" && (
            <Textarea
          ...
          {action === "approved" && (
            <p className="text-zinc-400 text-sm">
              Confirm approval of this member&apos;s ID documents.
            </p>
          )}
