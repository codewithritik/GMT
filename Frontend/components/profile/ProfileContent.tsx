...
export function ProfileContent({ isMember = false }: ProfileContentProps) {
  ...
  const onAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      toast.error(
        "Invalid file",
        "Only JPEG, PNG or WebP images are allowed (no GIF).",
      );
      return;
    }
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative -mt-16 sm:-mt-20 px-6 pb-6 flex flex-col sm:flex-row sm:items-end gap-4">
          <div className="relative shrink-0">
            <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-[#1e1e1e] bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center text-white font-bold text-3xl sm:text-4xl shadow-xl overflow-hidden">
              {showAvatar ? (
                <img
                  src={authAPI.profileAvatarUrl(user?.id, avatarMediaVersion)}
                  alt="Avatar"
                  className="w-full h-full object-cover"
                  onError={() => setAvatarFailed(true)}
                  key={`avatar-${user?.id}-${avatarMediaVersion}`}
                />
              ) : (
                initials
              )}
          ...
          <div className="flex-1 min-w-0">
            ...
            {isMember && (
              <span
                className={`inline-block mt-2 text-xs px-2.5 py-1 rounded-full font-medium capitalize ${idStatusBadge}`}>
                ID {idStatus}
              </span>
            )}
          </div>
      ...
      <PageHeader
        ...
        subtitle={
          isMember
            ? "Manage your personal details and emergency contact"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-white font-medium">
                  {p?.profile?.emergencyName ?? "—"}
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Phone</p>
                <p className="text-white font-medium">
                  {p?.profile?.emergencyPhone ?? "—"}
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Relation</p>
                <p className="text-white font-medium">
                  {p?.profile?.emergencyRelation ?? "—"}
                </p>
              </div>
            </div>
          </Card>
          <Card padding="lg">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs px-3 py-1.5 rounded-full font-semibold capitalize ${idStatusBadge}`}>
                  {idStatus}
                </span>
      ...
      <Modal
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            value={editForm.dob ?? ""}
            onChange={(e) =>
              setEditForm((f) => ({ ...f, dob: e.target.value || null }))
            }
          />
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setEditForm((f) => ({ ...f, gender: e.target.value || null }))
              }
          ...
          {isMember && (
            <>
              <Input
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, emergencyName: e.target.value }))
                }
              />
              <Input
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <LoadingButton loading={loading} onClick={handleEdit}>
              Save Changes
            </LoadingButton>
          </div>
        </div>
      ...
      <Modal
        isOpen={idResubmitOpen}
        onClose={() => setIdResubmitOpen(false)}
        title="Resubmit ID documents"
        size="md">
        <IdResubmitForm
          ...
          onUpload={async () => {
            ...
            if (!idDocFront) {
              setIdResubmitError(
                isPassport ? "Select passport image." : "Select front image.",
              );
              return;
            }
            ...
            try {
              ...
              if (!isPassport && idDocBack)
                formData.append("nic_back", idDocBack);
              ...
              toast.success(
                "Documents submitted",
                "Your ID documents have been resubmitted for verification.",
              );
...
function IdResubmitForm({
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={
          needsTwoFiles ? "grid grid-cols-1 sm:grid-cols-2 gap-4" : "space-y-4"
        }>
        <div>
          <label className="block text-sm font-medium text-zinc-300 mb-1">
