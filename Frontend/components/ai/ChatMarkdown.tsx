...
export function ChatMarkdown({
  ...
  const components: Components = {
    ...
    ul: ({ children }) => (
      <ul className="my-2 ml-4 list-disc space-y-1 marker:text-zinc-500">
        {children}
      </ul>
    ),
    ol: ({ children }) => (
      <ol className="my-2 ml-4 list-decimal space-y-1 marker:text-zinc-500">
        {children}
      </ol>
    ),
    li: ({ children }) => (
    ...
    h1: ({ children }) => (
      <p className="mb-1 mt-2 text-sm font-semibold text-zinc-100 first:mt-0">
        {children}
      </p>
    ),
    h2: ({ children }) => (
    ...
    h3: ({ children }) => (
      <p className="mb-1 mt-1.5 text-sm font-semibold text-zinc-200 first:mt-0">
        {children}
      </p>
    ),
    blockquote: ({ children }) => (
    ...
    code: ({ className: codeClass, children }) => {
      ...
      if (isBlock) {
        return (
          <pre className="my-2 max-h-48 overflow-x-auto overflow-y-auto rounded-lg border border-zinc-700/60 bg-zinc-950/90 p-2.5">
            <code className="text-[11px] leading-snug text-zinc-300">
              {children}
            </code>
          </pre>
        );
      ...
      return (
        <code className="rounded bg-zinc-950/90 px-1 py-0.5 font-mono text-[0.85em] text-zinc-200">
          {children}
        </code>
      );
    },
