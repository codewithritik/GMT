...
export function Navbar() {
  ...
  const excluded = NAVBAR_EXCLUDED_PATHS.some(
    (p) => pathname === p || pathname.startsWith(p + "/"),
  );
  const isSimulateRoute =
    pathname === "/simulate" || pathname.startsWith("/simulate/");
          ...
          <div className="relative w-7 h-7 flex items-center justify-center">
            <Menu
              ...
              className={cn(
                ...
                menuExpanded ? "opacity-0 scale-50" : "opacity-100 scale-100",
              )}
            />
            <X
              size={28}
              className={cn(
                "absolute transition-all duration-300",
                menuExpanded ? "opacity-100 scale-100" : "opacity-0 scale-50",
              )}
            />
          </div>
        </button>
      ...
      {!isSidebarRoute && (
        <div
          className={cn(
            ...
            isOpen
              ? "max-h-[28rem] py-6 border-white/10 opacity-100"
              ...
              <Link
                ...
                className={cn(
                  ...
                  isOpen
                    ...
                    : "text-transparent -translate-y-4 opacity-0",
                )}
