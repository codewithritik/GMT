...
const inputBaseClass =
  "w-full bg-zinc-800/80 border border-zinc-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-red-600 focus:border-red-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="px-4 py-2 text-sm bg-red-700 text-white rounded-lg hover:bg-red-600 transition-colors font-medium">
          {actionLabel}
        </button>
      )}
    </div>
...
export function ErrorAlert({ message, onRetry, className }: ErrorAlertProps) {
  return (
    <div
      className={cn(
        "p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-between gap-4",
        className,
      )}>
...
const btnVariantMap: Record<string, string> = {
  ...
  ghost:
    "bg-transparent text-zinc-400 hover:text-white hover:bg-zinc-800 border-transparent",
};
