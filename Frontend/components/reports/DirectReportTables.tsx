...
function truncNote(
  key: string,
  meta?: { directTruncated?: Trunc; directRowCap?: number },
) {
  if (!meta?.directTruncated?.[key]) return null;
  return (
    <p className="text-amber-400/90 text-xs mb-2">
      List truncated — showing up to {meta.directRowCap ?? "…"} rows. Download
      PDF for the same cap.
    </p>
  );
}
...
export function ReportMetaBar({
  ...
  data: {
    meta?: { generatedAt?: string; directRowCap?: number; piiMasked?: boolean };
  };
}) {
  const m = data.meta;
  ...
  return (
    <Card padding="lg" className={cn(reportSectionCard, "border-red-500/15")}>
      ...
      <p className="text-zinc-500 text-xs mb-4">
        Detailed records for operational review in the selected period.
      </p>
              ...
              <tbody>
                {d.payments.map((r: any, i: number) => (
                  <tr key={i} className={reportTableBodyRow}>
                    ...
                    <td
                      className={cn(reportTableCell, "text-right text-white")}>
                      ${Number(r.amount).toLocaleString("en-US")}
                    </td>
                    <td
                      className={cn(
                        reportTableCell,
                        "text-zinc-400 capitalize px-2",
                      )}>
                      {String(r.paymentMethod ?? "").replace("_", " ")}
                    </td>
                    <td className={cn(reportTableCell, "text-zinc-300")}>
                      {r.memberName ?? "—"}
                    </td>
                    <td className={cn(reportTableCell, "text-zinc-400")}>
                      {r.planName ?? "—"}
                    </td>
                    <td
                      className={cn(reportTableCell, "text-zinc-500 text-xs")}>
                      {r.receiptNumber ?? "—"}
                    </td>
                  </tr>
                ))}
      ...
      {d.newMemberRegistrations?.length > 0 && (
        <div className="mb-6">
          <h4 className="text-zinc-300 text-sm font-medium mb-2">
                ...
                {d.newMemberRegistrations.map((r: any, i: number) => (
                  <tr key={i} className={reportTableBodyRow}>
                    <td
                      ...
                      {fmtDt(r.registeredAt)}
                    </td>
                    <td className={cn(reportTableCell, "text-zinc-300")}>
                      {r.fullName}
                    </td>
                    <td
                      className={cn(reportTableCell, "text-zinc-400 text-xs")}>
                      {r.email}
                    </td>
                    <td className={cn(reportTableCell, "text-zinc-500")}>
                      {r.memberCode ?? "—"}
                    </td>
                  </tr>
                ))}
                ...
                {d.equipmentEvents.map((r: any, i: number) => (
                  <tr key={i} className={cn(reportTableBodyRow, "align-top")}>
                    ...
                    <td
                      className={cn(
                        reportTableCell,
                        "text-zinc-500 text-xs max-w-xs",
                      )}>
                      {r.description}
                    </td>
                  </tr>
