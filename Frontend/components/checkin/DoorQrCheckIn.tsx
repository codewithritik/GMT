...
export function DoorQrCheckIn({
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [myVisits, stats] = await Promise.all([
        opsAPI.myVisits(20),
        opsAPI.visitStats(),
      ]);
      ...
      setCapacity((prev) => ({
        ...prev,
        current: Number(stats?.activeNow ?? 0),
      }));
    } catch (err) {
    ...
    async (text: string) => {
      ...
      try {
        ...
        toast.success(
          ...
          action === "check_out"
            ? "Door unlocked — have a great day."
            : "Door unlocked — welcome in.",
        );
        await reload();
      } catch (err) {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined" && !window.isSecureContext) {
        throw new Error(
          "Camera requires HTTPS (or localhost). Open the site over HTTPS and try again.",
        );
      }
      ...
      const onZxingFrame = (result: any, err: any, controlsInCb: any) => {
        ...
        if (
          ...
          /no qr|not found|notfound/i.test(msg)
        )
          return;
        ...
        if (
          ...
          if (age > 6000) {
            ...
            setScanHint(
              "No QR detected yet. Increase screen brightness, move closer, and keep the QR centered.",
            );
          }
        }
        ...
        const fatal =
          /NotAllowed|Permission|denied|overconstrained|not supported|stream|Method not allowed/i.test(
            msg,
          );
        if (fatal && mountedRef.current) {
          toast.error(
            "Camera error",
            getErrorMessage(err) ?? "Failed to start camera stream.",
          );
          void stopCamera();
        }
      };
      ...
      try {
        ...
        const pickBack = (d: any) =>
          ...
          /rear camera/i.test(String(d.label ?? ""));
      ...
      toast.error(
        "Camera error",
        getErrorMessage(err) ?? "Failed to access camera.",
      );
      await stopCamera();
    } finally {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      ...
      <Card
        padding="lg"
        className={`flex flex-col gap-4 ${scannedIn ? "border-emerald-500/30" : ""}`}>
        ...
        {scanHint && <p className="text-amber-300 text-sm">{scanHint}</p>}
      ...
      <Card padding="lg">
        ...
        <div className="space-y-2">
          {log.map((l, i) => (
            <div
              key={i}
              className="flex min-w-0 flex-col gap-2 bg-zinc-800/30 rounded-xl p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
              <div className="flex min-w-0 items-center gap-3">
                {l.type === "in" ? (
