  ...
  async (error) => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const st = axios.isAxiosError(refreshError)
        ...
        if (st !== 429) {
          ...
          const publicRoutes = [
            ...
            "/~offline",
            "/member/register",
            "/member/register/personal-details",
            "/member/register/identity-verification",
            "/member/register/subscription",
            "/member/register/verify-email",
            "/member/register/dashboard",
            "/member/forgot-password",
            "/member/forgot-password/pin-code",
            "/member/forgot-password/new-password",
            "/member/forgot-password/success",
            "/member/verify-email",
...
export const authAPI = {
  ...
  logout: () =>
    apiClient.post("/auth/logout", undefined, { _noRetry: true } as any),
  ...
  sendVerificationEmail: () =>
    apiClient.post("/auth/send-verification", null, { _noRetry: true } as any),
  ...
  uploadMemberDocuments: (
    ...
    apiClient.post(`/auth/upload-id/${memberId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
      ...config,
    }),
  ...
  getIdDocumentBlob: (userId: string, type: "front" | "back") =>
    apiClient
      .get<Blob>(`/auth/admin/id-document/${userId}/${type}`, {
        responseType: "blob",
      })
      .then((r) => r.data),
...
export type ManagerActivityPoint = {
  label: string;
  visits: number;
  workouts: number;
  ptSessions: number;
...
export const opsAPI = {
  ...
  dashboard: (role: "admin" | "manager" | "trainer" | "member") =>
    apiClient
      .get<{ success: boolean; data: OpsDashboard }>(`/ops/dashboard/${role}`)
      .then((r) => r.data.data),
  dashboardAnalytics: (role: "manager" | "member") =>
    apiClient
      .get<{
        success: boolean;
        data: MemberDashboardAnalytics | ManagerDashboardAnalytics;
      }>(`/ops/dashboard/${role}/analytics`)
      .then((r) => r.data.data),
  reportSummary: (params?: {
    ...
    return apiClient
      .get<{ success: boolean; data: OpsReportSummary }>(
        "/ops/reports/summary",
        {
  ...
  downloadReportPdf: (params?: {
    ...
    apiClient
      .get<Blob>("/ops/reports/pdf", { params, responseType: "blob" })
      .then((r) => r.data),
  recentReports: () =>
  ...
  createPlan: (payload: {
    ...
    apiClient
      .post("/ops/subscriptions/plans", payload)
      .then((r) => r.data.data),
  updatePlan: (
    id: string,
    payload: Partial<{
    ...
    apiClient
      .patch(`/ops/subscriptions/plans/${id}`, payload)
      .then((r) => r.data.data),
  mySubscriptions: () =>
  ...
  branchCapacity: () =>
    apiClient
      .get("/ops/branch/capacity")
      .then((r) => r.data.data as { capacity: number }),
  systemStatus: () =>
    apiClient
      .get("/ops/system/status")
      .then((r) => r.data.data as { maintenanceMode: boolean }),
  myVisits: (limit = 20) =>
    apiClient
      .get("/ops/visits/me", { params: { limit } })
      .then((r) => r.data.data as any[]),
  visitStats: () =>
    apiClient
      .get("/ops/visits/stats")
      .then((r) => r.data.data as { activeNow: number; todayTotal: number }),
  visits: (limit = 100) =>
    apiClient
      .get("/ops/visits", { params: { limit } })
      .then((r) => r.data.data as any[]),
  ...
  ptBookingRules: () =>
    apiClient
      .get("/ops/pt-sessions/booking-rules")
      .then((r) => r.data.data as PtBookingRules),
  allPtSessions: () =>
  ...
  memberWorkoutPlans: (memberId: string) =>
    apiClient
      .get(`/ops/workouts/plans/member/${memberId}`)
      .then((r) => r.data.data as any[]),
  removeMyWorkoutPlan: (planId: string) =>
    apiClient
      .delete(`/ops/workouts/plans/me/${planId}`)
      .then((r) => r.data.data),
  assignWorkoutPlan: (payload: {
    memberId: string;
    name: string;
    description?: string;
    ...
    apiClient
      .post("/ops/workouts/plans/assign", payload)
      .then((r) => r.data.data),
  generateAiWorkoutPlan: (payload?: {
    memberId?: string;
    preferences?: AiWorkoutPlanPreferencesPayload;
  }) =>
    apiClient
      .post("/ops/workouts/plans/generate", payload ?? {})
      .then((r) => r.data.data),
  myWorkoutLogs: () =>
  ...
  activeWorkoutSession: () =>
    apiClient
      .get("/ops/workouts/sessions/active")
      .then((r) => r.data.data as any | null),
  startWorkoutSession: (payload: { planId?: string; notes?: string }) =>
    apiClient
      .post("/ops/workouts/sessions/start", payload)
      .then((r) => r.data.data),
  addWorkoutSessionEvent: (
    sessionId: string,
    payload: {
      eventType:
        | "paused"
    ...
    apiClient
      .post(`/ops/workouts/sessions/${sessionId}/events`, payload)
      .then((r) => r.data.data),
  stopWorkoutSession: (
    sessionId: string,
    payload?: {
  ...
  addMemberMetric: (
    ...
    apiClient
      .post(`/ops/metrics/member/${memberId}`, payload)
      .then((r) => r.data.data),
  ...
  resolveEquipmentEvent: (id: string) =>
    apiClient
      .patch(`/ops/equipment-events/${id}/resolve`)
      .then((r) => r.data.data),
  inventoryItems: () =>
  ...
  updateInventoryItem: (
    ...
    apiClient
      .patch(`/ops/inventory/items/${id}`, payload)
      .then((r) => r.data.data),
  inventoryTransactions: (itemId?: string) =>
    apiClient
      .get("/ops/inventory/transactions", {
        params: itemId ? { itemId } : undefined,
      })
      .then((r) => r.data.data as any[]),
  addInventoryTxn: (payload: {
    itemId: string;
    txnType: "restock" | "sale" | "adjustment" | "waste";
    qtyChange: number;
    reference?: string;
  }) =>
    apiClient
      .post("/ops/inventory/transactions", payload)
      .then((r) => r.data.data),

  // staff broadcast (audit trail only; replaces legacy in-app messages)
  staffBroadcast: (payload: {
    subject: string;
    body: string;
  ...
  trainers: () =>
    apiClient
      .get("/ops/trainers")
      .then((r) => r.data.data as Array<{ id: string; fullName: string }>),
  users: (role?: "admin" | "manager" | "trainer" | "member") =>
    apiClient
      .get("/ops/users", { params: role ? { role } : undefined })
      .then((r) => r.data.data as any[]),
  userById: (id: string) =>
    apiClient
      .get(`/ops/users/${id}`)
      .then((r) => r.data.data as SingleMemberResponse),
  createUser: (payload: {
    fullName: string;
    email: string;
  ...
  membersById: (id: string) =>
    apiClient
      .get(`/ops/members/${id}`)
      .then((r) => r.data.data as MemberProfilePageProps),
  updateMemberSubscriptionPlan: (
    memberId: string,
    subscriptionPlanId: string,
  ) =>
    apiClient
      .patch(`/ops/members/${memberId}/subscription-plan`, {
        subscriptionPlanId,
      })
      .then((r) => r.data.data),
  closures: () =>
  ...
  getWorkoutPlan: (planId: string) =>
    apiClient
      .get(`/ops/workouts/plans/${planId}`)
      .then((r) => r.data.data as any),
  patchWorkoutPlan: (
    planId: string,
    payload: { program: Record<string, unknown> },
  ) =>
    apiClient
      .patch(`/ops/workouts/plans/${planId}`, payload)
      .then((r) => r.data.data),

  // shifts
  shifts: (filters?: { staffId?: string; shiftDate?: string }) =>
    apiClient
      .get("/ops/shifts", { params: filters })
      .then((r) => r.data.data as any[]),
  myShifts: () =>
  ...
  simulateGenerateDoorOtp: (expiresInSec = 120) =>
    apiClient
      .post("/ops/simulate/door/otp", { expiresInSec })
      .then(
        (r) =>
          r.data.data as { token: string; code: string; expiresAt: string },
      ),
  simulateDoorScan: (payload: {
    token: string;
    code: string;
    personId?: string;
  }) =>
  ...
  simulateTrainerShift: (payload: {
    trainerId: string;
    action?: "in" | "out";
  }) =>
    apiClient
      .post("/ops/simulate/trainer-shift", payload)
      .then((r) => r.data.data),
  simulateAppointment: (payload: {
    memberId: string;
    trainerId: string;
    sessionDate: string;
    startTime: string;
    ...
    apiClient
      .post("/ops/simulate/appointment", payload)
      .then((r) => r.data.data),
  simulateVitals: (payload: {
    memberId: string;
    weightKg?: number;
    heightCm?: number;
    bmi?: number;
  ...
  publicSimulationBootstrap: () =>
    apiClient
      .get("/ops/simulate/public/bootstrap")
      .then(
        (r) =>
          r.data.data as {
            members: any[];
  ...
  publicSimulateGenerateDoorOtp: (expiresInSec = 120) =>
    apiClient
      .post("/ops/simulate/public/door/otp", { expiresInSec })
      .then(
        (r) =>
          r.data.data as { token: string; code: string; expiresAt: string },
      ),
  publicSimulateDoorScan: (payload: {
    token: string;
    code: string;
    personId: string;
  }) =>
    apiClient
      .post("/ops/simulate/public/door/scan", payload)
      .then((r) => r.data.data),
  publicSimulatePayment: (payload: {
    memberId: string;
    planId: string;
    paymentMethod?: "cash" | "card" | "bank_transfer" | "online";
  }) =>
    apiClient
      .post("/ops/simulate/public/payment", payload)
      .then((r) => r.data.data),
  publicSimulateCardPayment: (payload: {
    memberId: string;
    planId: string;
    cardPan: string;
    cardHolder?: string;
    ...
    apiClient
      .post("/ops/simulate/public/payment/card", payload)
      .then((r) => r.data.data),
  publicSimulateTrainerShift: (payload: {
    trainerId: string;
    action?: "in" | "out";
  }) =>
    apiClient
      .post("/ops/simulate/public/trainer-shift", payload)
      .then((r) => r.data.data),
  publicSimulateAppointment: (payload: {
    memberId: string;
    trainerId: string;
    sessionDate: string;
    startTime: string;
    ...
    apiClient
      .post("/ops/simulate/public/appointment", payload)
      .then((r) => r.data.data),
  publicSimulateVitals: (payload: {
    memberId: string;
    weightKg?: number;
    heightCm?: number;
    bmi?: number;
    ...
    apiClient
      .post("/ops/simulate/public/vitals", payload)
      .then((r) => r.data.data),
...
export const aiAPI = {
  ...
  insights: (question?: string) =>
    apiClient.post("/ai/insights", { question }, { timeout: 90_000 }).then(
      (r) =>
        r.data.data as {
          content: string;
          summary: string;
