...
export function validateReportDateRange(
  ...
  if (from && !isValidYmd(from)) {
    return {
      ok: false,
      message: "From date must be a valid date (YYYY-MM-DD).",
    };
  }
  if (to && !isValidYmd(to)) {
  ...
  if (from) {
    ...
    if (from < minYmd)
      return {
        ok: false,
        message: `From date cannot be more than ${REPORT_HISTORY_YEARS} years ago.`,
      };
  }
  if (to) {
    if (to > today)
    ...
    if (to < minYmd)
      return {
        ok: false,
        message: `To date cannot be more than ${REPORT_HISTORY_YEARS} years ago.`,
      };
  }
  if (from && to) {
    if (from > to)
