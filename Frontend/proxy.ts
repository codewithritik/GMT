...
export function proxy(request: NextRequest) {
  ...
  const bouncePath = BOUNCE_AUTHED_TO_DASHBOARD.some(
  ...
  if (hasSession && isProtected) {
    ...
    const wrongPortal = STAFF_PROTECTED.filter((p) => p !== myPortal).some(
      under,
    );
    // Also check member trying to access /trainer|/manager|/admin vice-versa
    const memberOnStaffPortal =
      role === "member" && STAFF_PROTECTED.some(under);
    const nonMemberOnMemberPortal =
      role !== "member" && MEMBER_PROTECTED.some(under);
